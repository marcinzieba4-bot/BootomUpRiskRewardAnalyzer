...
UCG Signal Model  v1
──────────────────────
UniCredit S.p.A. (BIT: UCG)  ·  Pan-European Banking (Italy, Germany, CEE)
Trough year: 2020 (pre-Orcel restructuring)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 83.03   # EUR, Borsa Italiana, 2026-08-21
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (5.0, 11, 55, "ECB cuts crush NII; Commerzbank integration stumbles; Italy risk premium returns"),
    "BASE": (7.6, 11, 82, "Guidance >EUR11bn delivered; Commerzbank closes and integrates cleanly"),
    "BULL": (8.8, 11, 100, "Commerzbank synergies + distributions re-rate UCG toward NW-European peers"),
    "XBULL": (10.3, 11, 118, "Pan-European champion premium; sector consolidation wave led from Milan"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E net profit, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Italy", "Home franchise — retail/corporate; the distribution machine's cash source", 5.2, 4.6, 5.6),
    ("Germany (HVB + CMZB)", "HVB plus the ~47.6% Commerzbank economic stake — the strategic bet", 3.0, 2.5, 3.6),
    ("Central & Eastern Europe", "Highest-ROE region; structural growth premium", 2.5, 2.2, 2.8),
    ("Other & rundown", "Russia wind-down + corporate center", 0.8, 0.5, 1.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Net profit growth", "% YoY",
      -5.0, 5.0, 15.0, 25.0, 24.0, True,
     "NII compression + provisions reverse the record run"),

    ("FY guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Guidance cut below EUR11bn"),

    ("CET1 ratio", "%",
      13.0, 14.5, 15.5, 16.5, 16.0, True,
     "Commerzbank consolidation absorbs the buffer"),

    ("Commerzbank progress", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Political blockage or hostile integration"),

    ("Cost discipline", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Integration costs + inflation break the cost line"),

    ("Capital distributions", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Buybacks paused to fund the German expansion"),

]
WEIGHTS = [0.20, 0.15, 0.15, 0.20, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Commerzbank optionality — ~47.6% secured, ECB approved, close possible Q4", 0.6, 0.25),
    ("Cross-border integration execution risk (political + operational)", -0.5, 0.2),
    ("Sector-leading distributions (buybacks + dividends)", 0.5, 0.2),
    ("Near three-year high after a multi-year re-rating run", -0.5, 0.2),
    ("NII sensitivity to the ECB cutting cycle", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 7.4   # FY2026E approx (guidance >EUR11bn / ~1.55bn shares), EUR
EPP_MIN_PE       = 6.0    # bank-trough floor — restructured, capital-rich franchise

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% profit (vs +24%; NII normalizes)"),
    (1, 2.0, "Score 2/4 (vs 3; guidance held not raised)"),
    (2, 15.0, "15% CET1 (vs 16; CMZB absorbs)"),
    (3, 2.0, "Score 2/4 (vs 3; slow integration)"),
    (4, 2.0, "Score 2/4 (vs 3; integration costs)"),
    (5, 2.0, "Score 2/4 (vs 3; buyback pause)"),
]
CONS_EPS_CAGR = 0.04
CONS_EXIT_PE  = 8.0
CONS_DIVIDEND = 2.4
...
print()
print("═" * W)
print(f"  UCG  ·  UniCredit S.p.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Pan-European Banking (Italy, Germany, CEE)")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E net profit, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  H1 2026: net profit EUR6.1bn +24% adj; FY guidance RAISED to >EUR11bn (~11.5bn ex-integration costs)")
...
print("""
  KEY TRIGGER: ECB rate cuts compressing NII exactly as Commerzbank integration
  costs land — earnings power falling while the strategic bet consumes capital
  and management attention. JOINT event: distributions cushion either alone.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2026E approx (guidance >EUR11bn / ~1.55bn shares), EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [bank-trough floor — restructured, capital-rich franchise]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and an 8x exit, the 2-yr price return is negative from a
  three-year high — the Commerzbank close is the swing factor in both directions.""")
...
print("""  Within ~3% of its 52-week high (EUR85.96); the market has already paid for a
  clean Commerzbank close — the asymmetry now runs against late buyers.""")
