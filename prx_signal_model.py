...
PRX Signal Model  v1
──────────────────────
Prosus N.V. (AMS: PRX)  ·  Global Consumer Internet (Tencent + ecosystems)
Trough year: 2022 (China tech crackdown + NAV-discount blowout)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 38.05   # EUR, Euronext Amsterdam, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (2.0, 13.5, 27, "Tencent derates on China macro/geopolitics; discount widens past 45%"),
    "BASE": (2.9, 14, 41, "Tencent steady; ecosystems compound profitably; buyback grinds the discount"),
    "BULL": (3.4, 15, 51, "Tencent re-rates + discount narrows below 30% on self-help"),
    "XBULL": (4.0, 16, 64, "China re-rating + ecosystem crown jewels (iFood, OLX) get valued standalone"),
}

# ── SEGMENT REVENUE BRIDGE (Gross asset value, USD bn) ────────────────────────────────
SEGMENTS = [
    ("Tencent (22.8% stake)", "~USD135bn — 76% of GAV; the equity story is a China proxy first", 134.7, 100.0, 165.0),
    ("Ecommerce ecosystems", "iFood, OLX, payments, etail, Just Eat Takeaway — ALL now profitable", 30.0, 22.0, 40.0),
    ("Other listed + net cash", "Delivery Hero et al + balance-sheet flexibility", 8.0, 6.0, 10.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("NAV discount", "%",
      45.0, 38.0, 30.0, 22.0, 35.0, False,
     "Discount widens back toward crisis levels on China fear"),

    ("Tencent momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "China consumer/gaming cycle rolls over"),

    ("Ecosystem profitability", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Food-delivery competition reopens the cash burn"),

    ("Ecosystem revenue growth", "% YoY",
      0.0, 10.0, 18.0, 26.0, 20.0, True,
     "Growth stalls in iFood/OLX core markets"),

    ("Buyback pace", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Buyback slows; discount loses its closing mechanism"),

    ("China regulatory / geopolitics", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "US-China escalation hits the Tencent stake directly"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Near its 52-week LOW after a ~40% derate — entry asymmetry improved", 0.5, 0.2),
    ("Single-asset concentration — Tencent is ~76% of GAV", -0.5, 0.25),
    ("Buyback machine funded by Tencent sales — mechanical discount closer", 0.5, 0.2),
    ("Geopolitical tail risk (US-China) on the core asset", -0.4, 0.2),
    ("Ecosystem inflection: all units profitable for the first time", 0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 2.9   # FY Mar-2026 core headline EPS approx, USD-linked
EPP_MIN_PE       = 12.0    # holding-co trough floor — Tencent look-through earnings at crisis multiple

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 38.0, "38% discount (vs 35; no narrowing)"),
    (1, 2.0, "Score 2/4 (vs 3; China cools)"),
    (2, 3.0, "Score 3/4 (vs 4; margins soften)"),
    (3, 12.0, "+12% growth (vs +20%)"),
    (4, 3.0, "Pace held (unchanged)"),
    (5, 2.0, "Score 2/4 (flat; tension persists)"),
]
CONS_EPS_CAGR = 0.1
CONS_EXIT_PE  = 13.0
CONS_DIVIDEND = 0.25
...
print()
print("═" * W)
print(f"  PRX  ·  Prosus N.V.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Global Consumer Internet (Tencent + ecosystems)")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (Gross asset value, USD bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  FY Mar-2026: ecosystem revenue USD7.3bn; ecosystem aEBITDA >USD1.1bn — all ecosystems profitable; NAV discount ~35%")
...
print("""
  KEY TRIGGER: a China macro/geopolitical shock derating Tencent WHILE the NAV
  discount simultaneously widens — the asset and the wrapper failing together,
  as in 2022. JOINT event: the buyback mechanically leans against the wrapper.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY Mar-2026 core headline EPS approx, USD-linked)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [holding-co trough floor — Tencent look-through earnings at crisis multiple]")
print(f"  {'─'*60}")
...
print("""
  At 10% look-through EPS CAGR and a 13x exit, the 2-yr return is solidly
  positive — the ~35% discount does the heavy lifting if Tencent merely holds.""")
...
print("""  Sits within ~5% of its 52-week LOW (EUR36.17) vs a EUR63.94 high — the only
  name in the batch entering near the bottom of its range, not the top.""")
