...
ISP Signal Model  v1
──────────────────────
Intesa Sanpaolo S.p.A. (BIT: ISP)  ·  Italian Retail, Wealth & Insurance Banking
Trough year: 2020 (COVID provisions + Italian sovereign stress)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 6.76   # EUR, Borsa Italiana, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (0.42, 11, 4.6, "ECB cuts crush NII; Italy spread widens; wealth fees stall"),
    "BASE": (0.59, 11, 6.5, "Guidance >EUR10bn delivered; wealth/insurance fees offset NII fade"),
    "BULL": (0.65, 12, 7.8, "Fee machine compounds; distributions re-rate the stock further"),
    "XBULL": (0.7, 13, 9.1, "Italian-champion premium; wealth consolidation led from Turin"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E gross income, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Banca dei Territori", "Domestic retail/SME franchise — the NII engine now fading with ECB cuts", 9.5, 8.6, 10.0),
    ("Wealth, AM & Protection", "Fisac/Eurizon/insurance — the fee annuity and the bull case", 4.8, 4.4, 5.3),
    ("IMI Corporate & Inv Bkg", "Corporate lending + markets; steady mid-cycle", 5.0, 4.4, 5.5),
    ("International Banks", "CEE + Egypt subsidiaries — growth premium, FX noise", 3.0, 2.7, 3.3),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Net income trajectory", "% YoY",
      -10.0, 0.0, 5.0, 12.0, 8.0, True,
     "NII compression outruns fee growth; provisions normalize up"),

    ("FY guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Guidance cut back below EUR10bn"),

    ("Net interest income resilience", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "ECB cutting cycle compresses deposit spreads faster than hedges roll"),

    ("Wealth & insurance fee momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Market downturn hits AuM and protection flows"),

    ("CET1 ratio", "%",
      12.0, 12.8, 13.3, 13.8, 13.5, True,
     "Capital erodes on RWA inflation + payouts"),

    ("Cost/income ratio", "%",
      55.0, 48.0, 44.0, 40.0, 42.5, False,
     "Inflation outpaces the efficiency program"),

]
WEIGHTS = [0.20, 0.15, 0.20, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Best-in-class wealth/insurance fee machine — least NII-dependent Italian bank", 0.6, 0.25),
    ("Within ~3% of 52-week high after a +40% run from the low", -0.7, 0.25),
    ("ECB cutting cycle is a direct NII headwind into 2027", -0.5, 0.2),
    ("Sector-leading distributions (5.5% yield + buybacks)", 0.4, 0.15),
    ("Italy sovereign-spread beta never fully leaves", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 0.555   # TTM reported, EUR
EPP_MIN_PE       = 6.0    # Italian-bank trough floor — the fee franchise survives spread cycles

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.0, "+2% net income (vs +8%; NII fades)"),
    (1, 2.0, "Score 2/4 (guidance held, not raised)"),
    (2, 2.0, "Score 2/4 (deposit spreads compress)"),
    (3, 2.0, "Score 2/4 (AuM flows cool)"),
    (4, 13.2, "13.2% CET1 (vs 13.5; payouts)"),
    (5, 44.0, "44% C/I (vs 42.5; cost creep)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 8.0
CONS_DIVIDEND = 0.37
...
print()
print("═" * W)
print(f"  ISP  ·  Intesa Sanpaolo S.p.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Italian Retail, Wealth & Insurance Banking")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E gross income, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  H1 2026: net income EUR5.6bn; FY guidance RAISED to >EUR10bn; next print Oct 30, 2026")
...
print("""
  KEY TRIGGER: the ECB cutting cycle compressing NII exactly as an Italian
  political/fiscal episode re-widens the BTP spread — earnings and multiple
  hit together. JOINT event: the wealth-fee annuity cushions either alone.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [Italian-bank trough floor — the fee franchise survives spread cycles]")
print(f"  {'─'*60}")
...
print("""
  At 3% EPS CAGR and an 8x exit, the 2-yr return is meaningfully negative from
  a 52-week high — the franchise is the best in Italy; the entry price is full.""")
...
print("""  Within ~3% of its 52-week high (EUR6.94) after a +40% run from EUR4.81;
  bank rallies of this size into a rate-cutting cycle historically fade.""")
