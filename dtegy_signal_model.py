#!/usr/bin/env python3
"""
DTEGY Signal Model  v1
──────────────────────
Deutsche Telekom AG (XETR: DTE · ADR: DTEGY)  ·  Telecom (T-Mobile US, Germany, Europe)
Trough year: 2020 (pre-TMUS/Sprint synergy realization)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 29.13   # EUR, Xetra listing (keyed DTEGY to avoid clash with DTE Energy), 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (1.4, 16, 22, "TMUS growth stalls on US competition; Germany/Europe stagnate; leverage bites"),
    "BASE": (1.9, 16, 30, "TMUS keeps taking share; EBITDA AL ~EUR47.5bn lands; dividend grows"),
    "BULL": (2.3, 16, 36, "TMUS re-rates + German fiber monetization; buyback accelerates"),
    "XBULL": (2.7, 16, 42, "Full TMUS premium + European consolidation re-rating"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("T-Mobile US", "~2/3 of group value; Q1 service revenue +11.3% — the growth engine", 82.0, 78.0, 85.0),
    ("Germany", "Market-leading incumbent; steady low-single-digit growth, fiber buildout", 26.0, 25.0, 27.0),
    ("Europe & Systems", "CEE mobile/fixed + T-Systems enterprise — stable, low growth", 16.0, 15.5, 16.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("TMUS service revenue growth", "% YoY",
      0.0, 3.0, 6.0, 10.0, 11.3, True,
     "US wireless competition intensifies; share gains reverse"),

    ("Adj EBITDA AL guidance", "EURbn",
      44.0, 46.0, 47.5, 49.0, 47.5, True,
     "Guidance cut on US or FX weakness"),

    ("Free cash flow AL", "EURbn",
      16.0, 18.0, 19.5, 21.0, 19.8, True,
     "Capex overruns absorb the cash generation"),

    ("Germany/Europe momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Domestic competition + regulation squeeze margins"),

    ("Dividend growth", "% YoY",
      0.0, 4.0, 8.0, 12.0, 11.0, True,
     "Payout growth pauses to protect leverage"),

    ("Leverage & buyback capacity", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Rating pressure forces deleveraging priority"),

]
WEIGHTS = [0.25, 0.15, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("T-Mobile US structural share-gain machine", 0.8, 0.3),
    ("Single-asset concentration — TMUS is ~2/3 of the value", -0.4, 0.2),
    ("European telecom stagnation drag ex-US", -0.5, 0.2),
    ("Growing dividend (+11%) + buyback at ~16x earnings", 0.4, 0.15),
    ("Trades 16% below 52-week high after TMUS drawdown", 0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 1.79   # TTM reported, EUR
EPP_MIN_PE       = 11.0    # telecom-incumbent floor — regulated cash flows, TMUS stake value

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% TMUS (vs +11.3%; competition)"),
    (1, 46.5, "EUR46.5bn (vs 47.5; FX drag)"),
    (2, 18.5, "EUR18.5bn FCF (vs 19.8)"),
    (3, 2.0, "Score 2/4 (vs 2.5; Europe flat)"),
    (4, 6.0, "+6% dividend (vs +11%)"),
    (5, 2.0, "Score 2/4 (vs 3; slower buyback)"),
]
CONS_EPS_CAGR = 0.05
CONS_EXIT_PE  = 14.0
CONS_DIVIDEND = 1.0
...
print()
print("═" * W)
print(f"  DTEGY  ·  Deutsche Telekom AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Telecom (T-Mobile US, Germany, Europe)")
...
print(f"  Q1 2026: guidance RAISED — adj EBITDA AL ~EUR47.5bn, FCF AL >EUR19.8bn; dividend +11% to EUR1.00")
...
print("""
  KEY TRIGGER: US wireless price war stalling T-Mobile's share-gain machine
  WHILE German/European operations stay stagnant — the single growth engine
  failing with nothing behind it. JOINT event: the domestic annuity cushions timing.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [telecom-incumbent floor — regulated cash flows, TMUS stake value]")
print(f"  {'─'*60}")
...
print("""
  At 5% EPS CAGR and a 14x exit, the 2-yr total return is roughly +9% with the
  growing dividend — a bond-plus profile, not a compounder, at today's price.""")
...
print("""  Lowest beta (0.75) in the coverage additions; the equity story is largely a
  US asset (TMUS) wrapped in a European wrapper with a 3.4% growing yield.""")
