...
ITX Signal Model  v1
──────────────────────
Industria de Diseño Textil, S.A. (Inditex) (BME: ITX)  ·  Global Fashion Retail (Zara)
Trough year: 2022 (post-COVID consumer squeeze, Russia exit)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 58.26   # EUR, BME Madrid, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (1.4, 14, 38, "Consumer recession + FX drag; cc growth turns negative; margin markdowns"),
    "BASE": (2.2, 25, 56, "Mid-single-digit cc growth; 61% gross margin held; space optimisation executes"),
    "BULL": (2.6, 27, 70, "Summer momentum sustains double-digit cc growth; margin expands further"),
    "XBULL": (3.1, 27, 84, "Global share gains accelerate + FX turns tailwind + online mix lifts margin"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Zara (incl. Zara Home)", "Flagship concept ~73% of sales — its health effectively IS group health", 29.5, 27.0, 31.5),
    ("Other brands", "Bershka, Stradivarius, Pull&Bear, Massimo Dutti, Oysho — younger/value formats", 11.0, 10.0, 12.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Constant-FX sales growth", "% YoY",
      -2.0, 4.0, 8.0, 12.0, 8.8, True,
     "Consumer downturn hits fast fashion; cc growth turns negative"),

    ("Summer trading window cc", "% YoY",
      0.0, 5.0, 10.0, 15.0, 11.5, True,
     "Early-summer momentum fades; the May trading window decelerates sharply"),

    ("Gross margin", "%",
      57.0, 59.5, 61.0, 62.5, 61.2, True,
     "Markdowns return as inventory builds; FX headwind compounds"),

    ("Net profit growth", "% YoY",
      -5.0, 3.0, 8.0, 14.0, 5.4, True,
     "Profit growth stalls on FX and opex deleverage"),

    ("Gross space growth", "%",
      1.0, 3.0, 5.0, 7.0, 5.0, True,
     "Store optimisation turns net-negative; expansion pauses"),

    ("FX headwind on sales", "pp",
      -4.0, -2.0, -1.0, 0.0, -1.0, True,
     "EUR strength deepens translation drag beyond -4pp"),

]
WEIGHTS = [0.25, 0.15, 0.20, 0.15, 0.10, 0.15]

STRUCTURAL_FACTORS = [
    ("Fully-integrated supply chain — fastest reaction cycle in retail", 1.0, 0.3),
    ("Net-cash balance sheet + 2.4% dividend yield", 0.5, 0.2),
    ("Valuation near 52-week high at ~29x trailing EPS", -0.7, 0.25),
    ("FX translation headwind on non-EUR sales (guided -1pp)", -0.4, 0.15),
    ("Middle East freight disruption exposure on logistics", -0.3, 0.1),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 2.02   # TTM reported, EUR
EPP_MIN_PE       = 18.0    # quality-retail floor — net cash, supply-chain moat

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% cc (vs +8.8%; consumer slows)"),
    (1, 6.0, "+6% (vs +11.5%; summer surge fades)"),
    (2, 60.0, "60% GM (vs 61.2%; modest markdown)"),
    (3, 3.0, "+3% (vs +5.4%; FX drag)"),
    (4, 3.0, "+3% space (vs +5%; slower rollout)"),
    (5, -2.0, "-2pp FX (vs -1pp; EUR firm)"),
]
CONS_EPS_CAGR = 0.07
CONS_EXIT_PE  = 22.0
CONS_DIVIDEND = 1.42
...
print()
print("═" * W)
print(f"  ITX  ·  Industria de Diseño Textil, S.A. (Inditex)  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Global Fashion Retail (Zara)")
...
print(f"  Q1 FY2026 actual: sales EUR8.75bn +5.8% (+8.8% cc), net profit EUR1.37bn +5.4%; next print Sep 9, 2026")
...
print("""
  KEY TRIGGER: a European/US consumer recession that turns constant-FX growth
  negative WHILE the EUR strengthens — double compression of reported sales and
  margin. JOINT event: the integrated supply chain absorbs single shocks well.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [quality-retail floor — net cash, supply-chain moat]")
print(f"  {'─'*60}")
...
print("""
  Even at a conservative 7% EPS CAGR and a 22x exit (well below today's ~29x),
  Inditex's net-cash model and store-optimisation program support total return near flat.""")
...
print("""  Stock sits within ~2% of its 52-week high after a strong Q1 print and an
  +11.5% cc start to summer — market is already paying for continued execution.""")
