...
AI Signal Model  v1
──────────────────────
Air Liquide S.A. (EPA: AI)  ·  Industrial Gases & Electronics
Trough year: 2020 (COVID industrial-volume trough)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 168.72   # EUR, Euronext Paris, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (5.0, 26, 130, "Industrial recession; electronics capex pause; margin program stalls"),
    "BASE": (5.9, 29, 171, "Mid-single-digit comparable growth + relentless margin accretion"),
    "BULL": (6.6, 31, 205, "Electronics/semi buildout accelerates; hydrogen projects convert"),
    "XBULL": (7.5, 32, 240, "Full energy-transition supercycle across gases, H2 and electronics"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Americas (Gas & Services)", "Largest region; US industrial + electronics fab buildout", 11.6, 10.8, 12.4),
    ("Europe (Gas & Services)", "Margin-recovery story post energy crisis; healthcare stable", 9.6, 9.0, 10.2),
    ("Asia-Pacific (G&S)", "China industrial merchant + electronics; the cyclical swing factor", 6.4, 5.9, 7.0),
    ("Middle East & Africa", "Small but growing on energy-transition projects", 0.9, 0.8, 1.0),
    ("Engineering & Global Mkts", "Project pipeline converter — leading indicator for future G&S", 1.5, 1.3, 1.8),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Comparable sales growth", "% YoY",
      0.0, 2.5, 4.5, 6.5, 3.5, True,
     "Industrial production rolls over; merchant volumes shrink"),

    ("Operating margin improvement", "bp YoY",
      0.0, 50.0, 100.0, 150.0, 110.0, True,
     "Efficiency program exhausted; energy passthrough dilutes"),

    ("Electronics momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Semi fab capex digestion pauses the record backlog"),

    ("Industrial merchant pricing", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Pricing power fades as volumes soften"),

    ("Investment backlog conversion", "EURbn",
      1.5, 2.2, 2.8, 3.4, 2.9, True,
     "Decisions slow; hydrogen projects deferred"),

    ("EPS growth", "% YoY",
      0.0, 4.0, 8.0, 12.0, 6.5, True,
     "FX + energy costs eat the operating leverage"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Oligopoly moat — take-or-pay contracts, 30-yr customer lock-ins", 0.8, 0.25),
    ("~30x trailing EPS in the upper half of its 52-week range", -0.7, 0.25),
    ("Margin-accretion machine — +110bp H1, targets reaffirmed through 2027", 0.3, 0.2),
    ("Electronics + energy-transition backlog at records", 0.3, 0.15),
    ("Defensive compounder premium — it never trades cheap", -0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 5.55   # TTM reported, EUR
EPP_MIN_PE       = 20.0    # gas-oligopoly floor — contracted volumes survive any recession

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.5, "+2.5% comparable (vs +3.5%)"),
    (1, 70.0, "+70bp margin (vs +110bp)"),
    (2, 3.0, "Score 3/4 (vs 4; semi digestion)"),
    (3, 2.0, "Score 2/4 (vs 3; pricing fades)"),
    (4, 2.4, "EUR2.4bn decisions (vs 2.9)"),
    (5, 4.5, "+4.5% EPS (vs +6.5%)"),
]
CONS_EPS_CAGR = 0.06
CONS_EXIT_PE  = 26.0
CONS_DIVIDEND = 3.36
...
print()
print("═" * W)
print(f"  AI  ·  Air Liquide S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Industrial Gases & Electronics")
...
print(f"  H1 2026: revenue ~EUR14bn, +4.3% comparable; Q2 +3.5% (accel. from Q1 +1.9%); margin +110bp; 2026-27 targets reaffirmed")
...
print("""
  KEY TRIGGER: a global industrial recession hitting merchant volumes WHILE
  semiconductor fab capex pauses the electronics backlog — the cyclical layer
  and the growth layer failing together. JOINT event: take-or-pay contracts
  and healthcare gases cushion the core through either.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [gas-oligopoly floor — contracted volumes survive any recession]")
print(f"  {'─'*60}")
...
print("""
  At 6% EPS CAGR and a 26x exit (vs ~30x today), the 2-yr return is slightly
  negative — the compounding is real, and it is fully paid for at EUR169.""")
...
print("""  Lowest-volatility name in the batch (16%, beta 0.80) — the classic
  sleep-at-night compounder, which is exactly why the multiple never rests.""")
