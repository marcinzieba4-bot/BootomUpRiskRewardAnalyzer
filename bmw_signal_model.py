...
BMW Signal Model  v1
──────────────────────
Bayerische Motoren Werke AG (XETR: BMW)  ·  Premium Automobiles & Motorcycles
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 58.4   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (7.0, 5.5, 39, "China JV impairments deepen; Neue Klasse launch stumbles; margins stay ~1%"),
    "BASE": (10.0, 6.0, 60, "Margin 1-3% trough year, then Neue Klasse volume ramps into 2027"),
    "BULL": (12.0, 7.0, 84, "Neue Klasse succeeds — EV cost/software parity; China stabilizes"),
    "XBULL": (14.0, 8.0, 112, "Full product-cycle re-rating; premium-EV leadership established"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Automotive", "BMW/MINI/Rolls-Royce — Neue Klasse is the biggest product bet since the 3-series", 115.0, 108.0, 120.0),
    ("Financial Services", "Captive finance book — stable, rate-sensitive", 17.0, 16.0, 18.0),
    ("Motorcycles", "Niche but profitable", 3.2, 3.0, 3.5),
]
...
SIGNALS = [
    ("China momentum", "/4",
     ...
     "Share loss to local EV luxury becomes permanent"),

    ("Automotive EBIT margin", "%",
      1.0, 3.0, 5.0, 7.0, 2.3, True,
     "Tariffs + JV depreciation keep margins at the floor"),

    ("Neue Klasse launch execution", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "The product bet lands flat against Chinese EV pace"),

    ("Europe / Americas momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Western demand cracks before the product cycle arrives"),

    ("Balance sheet & FS book", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Credit losses + residual values turn in a downturn"),

    ("Capital returns", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Dividend rebased below EUR4 to protect investment"),

]
WEIGHTS = [0.20, 0.20, 0.20, 0.15, 0.10, 0.15]

STRUCTURAL_FACTORS = [
    ("~5.6x earnings + 7.5% dividend at the bottom of the range", 0.4, 0.2),
    ("China structural pressure — -30% retail is a franchise question", -0.6, 0.25),
    ("Neue Klasse: a genuine product-cycle catalyst launching NOW", 0.4, 0.2),
    ("Restructuring burden (~1.25pp) is one-off-ish and self-chosen", 0.2, 0.15),
    ("Tariff exposure on the US/EU axis", -0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 10.39   # TTM reported, EUR
EPP_MIN_PE       = 4.0    # premium-auto trough floor — brand + FS book + net cash industrial ops

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 1.0, "Score 1/4 (China stays lost)"),
    (1, 1.5, "1.5% margin (low end of guide)"),
    (2, 2.0, "Score 2/4 (vs 3; slow ramp)"),
    (3, 2.0, "Score 2/4 (vs 2.5; macro drag)"),
    (4, 3.0, "Score 3/4 (book holds)"),
    (5, 2.0, "Score 2/4 (dividend trimmed)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 5.0
CONS_DIVIDEND = 4.4
...
print()
print("═" * W)
print(f"  BMW  ·  Bayerische Motoren Werke AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Premium Automobiles & Motorcycles")
...
print(f"  Q2 2026 (Aug 5): revenue EUR31.3bn, EBT -35% to EUR1.7bn; auto EBIT margin 2.3%; China retail -30.2%; FY margin guidance 1-3% incl. ~1.25pp restructuring burden")
...
print("""
  KEY TRIGGER: a Neue Klasse commercial disappointment landing WHILE China
  keeps bleeding — the turnaround product failing into the structural decline.
  JOINT event: the western business and FS book fund years of iteration.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [premium-auto trough floor — brand + FS book + net cash industrial ops]")
print(f"  {'─'*60}")
...
print("""
  At 3% EPS CAGR and a 5x exit, the 2-yr total return is ~+10%, all dividend —
  the Neue Klasse verdict, not valuation, decides this stock's next regime.""")
...
print("""  Within ~3.5% of its 52-week low (EUR56.40) with the biggest product launch
  in two decades underway: maximum pessimism meeting maximum product news.""")
