...
CS Signal Model  v1
──────────────────────
AXA S.A. (EPA: CS)  ·  Global Insurance (P&C-led)
Trough year: 2020 (COVID BI losses + dividend restraint)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 44.91   # EUR, Euronext Paris, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (3.2, 10, 32, "Mega nat-cat year + reserve additions; soft P&C pricing turn"),
    "BASE": (4.0, 11, 44, "High end of 2024-26 plan lands; combined ratio holds low-90s"),
    "BULL": (4.6, 12, 55, "Pricing stays firm; buybacks compound; re-rating toward Zurich"),
    "XBULL": (5.4, 13, 70, "Hard market extends + capital-light growth re-rates the group"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E underlying earnings, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Property & Casualty", "AXA XL + European retail P&C — the earnings engine, pricing-firm", 4.9, 4.4, 5.2),
    ("Life & Health", "Protection-tilted book; capital-light shift ongoing", 3.0, 2.8, 3.2),
    ("Asset Mgmt & Banking", "Post-BNP AM deal — smaller but stable fee layer", 0.5, 0.4, 0.6),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Underlying EPS growth", "% YoY",
      0.0, 4.0, 7.0, 10.0, 8.0, True,
     "Nat-cat + soft pricing reverse the compounding"),

    ("P&C combined ratio", "%",
      97.0, 94.0, 92.0, 90.0, 91.0, False,
     "Cat load + claims inflation push CoR toward 95+"),

    ("Nat-cat budget discipline", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "A US hurricane/EU flood cluster blows the budget"),

    ("Solvency II ratio", "%",
      190.0, 205.0, 215.0, 225.0, 216.0, True,
     "Market shock + payouts erode the buffer"),

    ("Capital return (div + buyback)", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Buyback paused to fund M&A or absorb losses"),

    ("Life & Health momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Protection margins compress; outflows in savings"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~12x earnings with a 5.2% yield — undemanding for record results", 0.5, 0.2),
    ("At the 52-week high after a +23% run — plan success is priced", -0.5, 0.25),
    ("Climate/nat-cat tail grows structurally every year", -0.4, 0.2),
    ("Disciplined capital management post-XL era", 0.3, 0.2),
    ("2027 plan reset is an unknown — targets roll over this year", -0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 3.71   # TTM underlying, EUR
EPP_MIN_PE       = 8.0    # insurance-trough floor — diversified book, fortress solvency

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 4.0, "+4% EPS (vs +8%; cat load normal)"),
    (1, 93.0, "93% CoR (vs 91; claims inflation)"),
    (2, 2.0, "Score 2/4 (vs 3; budget pressured)"),
    (3, 208.0, "208% SII (vs 216; payouts)"),
    (4, 3.0, "Return held (unchanged)"),
...
print()
print("═" * W)
print(f"  CS  ·  AXA S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Global Insurance (P&C-led)")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E underlying earnings, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  FY2025: record results — net income +26%, underlying EPS +8%; management guides to HIGH END of 2024-26 targets")
...
print("""
  KEY TRIGGER: a record nat-cat year (US hurricane + EU flood cluster) landing
  in the SAME year the P&C pricing cycle softens — losses up, rate down.
  JOINT event: reinsurance cover + solvency absorb either alone.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (TTM underlying, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [insurance-trough floor — diversified book, fortress solvency]")
print(f"  {'─'*60}")
...
print("""
  At 5% EPS CAGR and a 10x exit, the 2-yr price return is roughly flat and the
  10.6% cumulative yield carries the case — a coupon with drift, at the high.""")
...
print("""  Within 2% of its 52-week high (EUR45.66); insurers rarely hold re-rating
  gains through the first bad cat season — the yield is the durable part.""")
