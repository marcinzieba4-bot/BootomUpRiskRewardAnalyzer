...
AD Signal Model  v1
──────────────────────
Koninklijke Ahold Delhaize N.V. (AMS: AD)  ·  Food Retail — US East Coast · Benelux · CEE
Trough year: 2020 (COVID ops strain, panic multiple)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 31.0   # EUR, Euronext Amsterdam, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (2.1, 10, 21, "US consumer cracks; SNAP/pharmacy drags deepen; price war erodes the 4% margin"),
    "BASE": (2.65, 12, 32, "~4% margin held; mid-to-high-SD EPS growth cc; FCF >= EUR2.3bn"),
    "BULL": (2.95, 13, 38, "US headwinds lap; Bol/online scales profitably; multiple mean-reverts"),
    "XBULL": (3.3, 14, 46, "Full defensive re-rating + US market-share gains"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E net sales, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("United States", "Food Lion, Stop & Shop, Hannaford — ~60% of sales; SNAP/pharmacy headwinds", 57.0, 55.0, 59.0),
    ("Europe", "Albert Heijn, Delhaize, CEE — steady, market-leading positions", 25.5, 24.5, 26.5),
    ("Bol & online", "Benelux e-commerce platform + omnichannel grocery", 12.5, 11.5, 13.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Comparable sales growth", "% YoY",
      -1.0, 1.0, 2.5, 4.0, 1.2, True,
     "US volume weakness broadens past SNAP/pharmacy"),

    ("Underlying operating margin", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Price investment cracks the ~4% line"),

    ("Guidance delivery", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "EPS-growth corridor abandoned"),

    ("US consumer environment", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "SNAP cuts + trade-down hit basket sizes harder"),

    ("Free cash flow", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Capex + working capital eat the EUR2.3bn floor"),

    ("Bol & online economics", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "E-commerce growth stalls or margins dilute"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("AT its 52-week low — a defensive staple priced at a cyclical's discount", 0.4, 0.2),
    ("~12x earnings + 3.9% dividend + rolling buyback", 0.3, 0.2),
    ("US headwinds (SNAP, pharmacy, deflation) are real and multi-quarter", -0.4, 0.25),
    ("Grocery growth ceiling — comps compound at 1-2%, not 5%", -0.4, 0.2),
    ("Beta 0.50 — the most defensive earnings stream in this coverage", 0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 2.51   # TTM underlying, EUR
EPP_MIN_PE       = 9.0    # grocer floor — food demand is the last thing consumers cut

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 0.8, "+0.8% comps (vs +1.2; drags persist)"),
    (1, 3.0, "Score 3/4 (margin held)"),
    (2, 2.0, "Score 2/4 (low end of corridor)"),
    (3, 2.0, "Score 2/4 (headwinds persist)"),
    (4, 3.0, "Score 3/4 (FCF floor holds)"),
    (5, 2.0, "Score 2/4 (vs 3; slower)"),
]
CONS_EPS_CAGR = 0.04
CONS_EXIT_PE  = 11.0
CONS_DIVIDEND = 1.2
...
print()
print("═" * W)
print(f"  AD  ·  Koninklijke Ahold Delhaize N.V.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Food Retail — US East Coast · Benelux · CEE")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E net sales, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026 (Aug 5): net sales EUR23.2bn +1.9% cc, comps +1.2%; uEPS EUR0.63 (slight miss); FY guidance REITERATED: ~4% margin, mid-to-high-SD EPS growth cc, FCF >= EUR2.3bn")
...
print("""
  KEY TRIGGER: a genuine US consumer recession stacking on the existing
  SNAP/pharmacy drags WHILE a grocery price war forces margin investment —
  volume and margin failing together in the 60%-of-sales market. JOINT event:
  Europe and Bol are structurally unaffected by US food-stamp policy.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [grocer floor — food demand is the last thing consumers cut]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and an 11x exit, the 2-yr total return is ~+4% — the model
  cannot make a 1%-comp grocer clear a 15%/yr hurdle, even from the low.""")
...
print("""  AT the 52-week low (EUR30.82) with guidance intact: the market is pricing
  the US headwinds as permanent. Beta 0.50 — this is where defensives hide.""")
