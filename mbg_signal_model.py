...
MBG Signal Model  v1
──────────────────────
Mercedes-Benz Group AG (XETR: MBG)  ·  Premium & Luxury Automobiles · Vans · Mobility
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 45.0   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (3.0, 6, 18, "China collapse deepens; tariffs + EV price war crush margins"),
    "BASE": (5.2, 9, 47, "Cars RoS 3-5% delivered; China stabilizes low; buyback continues"),
    "BULL": (6.5, 10, 65, "CLA/GLC EV cycle lands; China L-W strategy works; RoS back toward 8%"),
    "XBULL": (8.0, 11, 88, "Full luxury-strategy vindication + China recovery"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Mercedes-Benz Cars", "Core premium/luxury — China -30% is the open wound", 105.0, 98.0, 110.0),
    ("Mercedes-Benz Vans", "Commercial + camper vans — quietly excellent margins", 19.0, 18.0, 20.5),
    ("Mercedes-Benz Mobility", "Financial services — rate-sensitive, stable", 25.0, 24.0, 26.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Global deliveries trend", "% YoY",
      -10.0, -3.0, 2.0, 6.0, -8.0, True,
     "Volume decline broadens beyond China"),

    ("China momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 1.0, True,
     "Local competitors take the luxury segment for good"),

    ("North America / Europe momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Tariffs + macro end the western offset"),

    ("Cars return on sales", "%",
      2.0, 4.0, 6.0, 8.0, 4.1, True,
     "Pricing discipline breaks under EV competition"),

    ("BEV transition execution", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "+50% BEV growth stalls; software/platform delays return"),

    ("Capital returns", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Buyback + dividend cut to protect investment budget"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~8.7x earnings + 7.8% dividend yield — deep-value optics", 0.4, 0.2),
    ("China structural share loss to local EV luxury — may be permanent", -0.6, 0.25),
    ("Luxury (Top-End) strategy has not delivered pricing resilience", -0.3, 0.2),
    ("US/EU tariff exposure on the export model", -0.3, 0.15),
    ("-40% from its 52-week high; expectations fully reset", 0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 5.2   # TTM approx, EUR
EPP_MIN_PE       = 5.0    # auto-trough floor — brand + Vans + Mobility book value underpin

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, -5.0, "-5% deliveries (vs -8%; slow fade)"),
    (1, 1.0, "Score 1/4 (China stays lost)"),
    (2, 2.0, "Score 2/4 (vs 3; tariffs bite)"),
    (3, 3.5, "3.5% RoS (vs 4.1)"),
    (4, 2.0, "Score 2/4 (vs 3; EV race costly)"),
    (5, 2.0, "Score 2/4 (returns trimmed)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 7.0
CONS_DIVIDEND = 3.5
...
print()
print("═" * W)
print(f"  MBG  ·  Mercedes-Benz Group AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Premium & Luxury Automobiles · Vans · Mobility")
...
print(f"  Q2 2026: global deliveries -8% (China -30%, North America +13%, Europe +4%); BEV deliveries +50%; Cars RoS guidance 3-5% maintained")
...
print("""
  KEY TRIGGER: already partially triggered — China luxury share loss (-30%)
  IS the bear case in motion. Full BEAR needs the western offset (NA +13%,
  Europe +4%) to fail too, via tariffs or recession. JOINT event: Vans and
  Mobility earnings are the non-Cars cushion.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [auto-trough floor — brand + Vans + Mobility book value underpin]")
print(f"  {'─'*60}")
...
print("""
  At 3% EPS CAGR and a 7x exit, the 2-yr return is roughly flat INCLUDING the
  7.8% yield — cheapness alone is not a catalyst while China bleeds.""")
...
print("""  A EUR35-74.90 52-week range and a -29% YTD: the market is repricing the
  franchise's China annuity to zero in real time. The yield pays you to wait,
