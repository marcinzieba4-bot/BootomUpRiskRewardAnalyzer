#!/usr/bin/env python3
"""
ADYEN Signal Model  v1
──────────────────────
Adyen N.V. (AMS: ADYEN)  ·  Enterprise Payments Platform
Trough year: 2023 (growth-scare crash)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 1054.6   # EUR, Euronext Amsterdam, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (28.0, 20, 560, "Growth decelerates below 15%; enterprise pricing pressure; margin ambition slips"),
    "BASE": (44.0, 24, 1056, "21-23% cc growth lands; EBITDA margin grinds toward the long-term ambition"),
    "BULL": (54.0, 26, 1400, "Platform strategy (Talon.One, Orb, banking) compounds wallet share"),
    "XBULL": (65.0, 28, 1820, "Full financial-OS re-rating — payments plus software plus banking at scale"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E net revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Digital", "Global e-commerce/platform giants — the volume core", 1.6, 1.45, 1.75),
    ("Unified Commerce", "Omnichannel enterprise retail — highest strategic stickiness", 0.75, 0.68, 0.85),
    ("Platforms", "Embedded payments/banking for SaaS platforms — fastest growing", 0.4, 0.35, 0.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Net revenue growth (cc)", "% YoY",
      8.0, 15.0, 21.0, 27.0, 21.0, True,
     "Enterprise wallet-share gains stall; take-rate pressure"),

    ("EBITDA margin (underlying)", "%",
      38.0, 44.0, 49.0, 53.0, 50.0, True,
     "Investment phase reopens; margin ambition deferred"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Growth guided down as volumes normalize"),

    ("Volume & wallet-share momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Existing-merchant expansion (the engine) slows"),

    ("Platform expansion (M&A)", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Talon.One/Orb integrations dilute focus"),

    ("Competitive field (Stripe et al)", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Price competition reaches the enterprise tier"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~34% below its 52-week high after the 2026 derate", 0.3, 0.2),
    ("~30x trailing for 20%+ growth at 50% margins — premium, not euphoric", -0.4, 0.25),
    ("Single-platform architecture — the structural moat vs stitched rivals", 0.4, 0.2),
    ("Enterprise customer base: churn-resistant, volume-compounding", 0.3, 0.15),
    ("Payments competition and take-rate gravity never sleep", -0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 35.44   # TTM reported, EUR
EPP_MIN_PE       = 18.0    # quality-growth floor — 50%-margin platforms do not trade at market multiples

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 16.0, "+16% cc (vs +21; normalization)"),
    (1, 47.0, "47% margin (vs 50)"),
    (2, 3.0, "Score 3/4 (vs 4; held)"),
    (3, 2.0, "Score 2/4 (vs 3; slower expansion)"),
    (4, 2.0, "Score 2/4 (vs 3; integration tax)"),
    (5, 2.0, "Score 2/4 (vs 2.5)"),
]
CONS_EPS_CAGR = 0.15
CONS_EXIT_PE  = 26.0
CONS_DIVIDEND = 0.0
...
print()
print("═" * W)
print(f"  ADYEN  ·  Adyen N.V.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Enterprise Payments Platform")
...
print(f"  H1 2026 (Aug 13): net revenue EUR1.3bn +21% cc; EBITDA EUR642M at ~50% underlying margin; EUR804bn volume; FY guidance RAISED to 21-23% cc (Talon.One + Orb)")
...
print("""
  KEY TRIGGER: an enterprise-tier price war (Stripe/local rails) arriving as
  volume growth normalizes — take-rate and volume compressing together. JOINT
  event: the single-platform cost position lets Adyen win any rational price war.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [quality-growth floor — 50%-margin platforms do not trade at market multiples]")
print(f"  {'─'*60}")
...
print("""
  At 15% EPS CAGR and a 26x exit, the 2-yr return is ~+16% — rare among richly
  valued names: conservative assumptions still clear a positive return.""")
...
print("""  38% vol and a EUR772-1,601 range: the market re-prices this platform's
  terminal value violently on small growth deltas. The +16% results-day jump cut both ways all year.""")
