...
DHL Signal Model  v1
──────────────────────
DHL Group (XETR: DHL)  ·  Express · Freight Forwarding · Supply Chain · eCommerce · Post
Trough year: 2023 (post-COVID freight normalization)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 56.45   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (2.4, 11, 26, "Global trade contracts; Express yields fall; guidance reset down"),
    "BASE": (3.4, 15, 51, "EBIT >EUR6.5bn lands; trade flows re-route but keep moving"),
    "BULL": (3.9, 16, 62, "Volume + yield compound; buyback extends; margin structurally higher"),
    "XBULL": (4.6, 17.5, 80, "Global-trade upcycle + e-commerce logistics premium re-rating"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Express", "Time-definite international — the margin crown jewel", 27.0, 25.0, 29.0),
    ("Global Forwarding, Freight", "Air/ocean/road forwarding — rate-cycle exposed, asset-light", 21.0, 19.0, 23.5),
    ("Supply Chain", "Contract logistics — steady grower, sticky contracts", 18.5, 17.5, 19.5),
    ("eCommerce", "Parcel networks outside Germany — structural volume growth", 7.5, 7.0, 8.2),
    ("Post & Parcel Germany", "Mail decline vs parcel growth; regulated drag", 17.5, 17.0, 18.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Group EBIT growth", "% YoY",
      -5.0, 5.0, 15.0, 25.0, 30.0, True,
     "Trade contraction reverses the operating leverage"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Guidance reset down on volumes"),

    ("Express momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Yield erosion as capacity outruns demand"),

    ("Free cash flow", "EURbn",
      1.8, 2.4, 3.0, 3.6, 3.0, True,
     "Working capital + capex absorb the conversion"),

    ("Global trade environment", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Tariff escalation shrinks cross-border volumes outright"),

    ("Capital returns (div + buyback)", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Buyback paused on a downturn"),

]
WEIGHTS = [0.20, 0.15, 0.15, 0.15, 0.20, 0.15]

STRUCTURAL_FACTORS = [
    ("Within ~3% of its 52-week (and near record) high after a +50% run", -0.6, 0.25),
    ("Global trade/tariff cycle — volumes are the macro, not the company", -0.5, 0.25),
    ("Structural e-commerce logistics growth across every segment", 0.3, 0.2),
    ("~17.6x with a 3.4% dividend + upsized buyback", 0.2, 0.15),
    ("Express network moat — only 3 global integrators exist", 0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 3.2   # TTM approx, EUR
EPP_MIN_PE       = 10.0    # logistics-trough floor — the Express network is irreplaceable

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 8.0, "+8% EBIT (vs +30%; base effect)"),
    (1, 3.0, "Score 3/4 (vs 4; held)"),
    (2, 2.0, "Score 2/4 (vs 3; yields soften)"),
    (3, 2.6, "EUR2.6bn FCF (vs 3.0)"),
    (4, 2.0, "Score 2/4 (vs 2.5; tariffs bite)"),
    (5, 3.0, "Score 3/4 (vs 4; slower pace)"),
]
CONS_EPS_CAGR = 0.06
CONS_EXIT_PE  = 14.0
CONS_DIVIDEND = 1.9
...
print()
print("═" * W)
print(f"  DHL  ·  DHL Group  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Express · Freight Forwarding · Supply Chain · eCommerce · Post")
...
print(f"  Q2 2026 (Aug 5): revenue EUR22.4bn +13%, EBIT EUR1.9bn +30%; FY EBIT guidance RAISED to >EUR6.5bn; buyback upsized to EUR6.5bn through 2027")
...
print("""
  KEY TRIGGER: a genuine global-trade contraction (tariff escalation shrinking
  cross-border volumes outright) — the one scenario where Express, Forwarding
  and eCommerce all shrink together. JOINT event: Supply Chain contracts and
  German parcel volumes are domestic cushions.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [logistics-trough floor — the Express network is irreplaceable]")
print(f"  {'─'*60}")
...
print("""
  At 6% EPS CAGR and a 14x exit, the 2-yr return is roughly flat — the +30%
  EBIT quarter is cyclical amplitude, and the price now assumes it repeats.""")
...
print("""  Within ~3% of a record high after a +50% run from EUR36.99: logistics is a
  GDP-times-beta business, and the market has just re-remembered the upside half.""")
