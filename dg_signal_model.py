...
DG Signal Model  v1
──────────────────────
VINCI S.A. (EPA: DG)  ·  Concessions, Energy Solutions & Construction
Trough year: 2020 (COVID traffic collapse)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 120.25   # EUR, Euronext Paris, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (7.8, 11, 86, "French recession + autoroute expiry fears; Energies margin stalls"),
    "BASE": (9.2, 13, 120, "Guidance delivered: growth in revenue, op income and net income"),
    "BULL": (10.0, 14, 140, "Airports + Energies compound; expiry overhang priced rationally"),
    "XBULL": (11.5, 16, 184, "Infrastructure re-rating; concession renewals/extensions surprise"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Concessions", "Autoroutes + VINCI Airports (ANA, Gatwick...) — the cash machine", 12.0, 11.0, 12.8),
    ("VINCI Energies", "Electrification/data-center engineering — the growth engine", 22.5, 21.0, 24.0),
    ("Cobra IS", "Spanish energy engineering + renewables EPC — high growth", 7.5, 6.8, 8.5),
    ("Construction", "Global contracting — thin margin, huge order book", 32.0, 30.0, 33.5),
]
...
SIGNALS = [
    ("Revenue growth", "% YoY",
     ...
     "French macro + construction slowdown flatten the topline"),

    ("Order book", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Record backlog stops converting; awards dry up"),

    ("Concessions traffic & tariffs", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Traffic stalls; tariff formulae squeezed politically"),

    ("Energies/Cobra margin", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Electrification demand pauses; pricing competition returns"),

    ("Net income growth", "% YoY",
      -2.0, 2.0, 6.0, 10.0, 5.0, True,
     "Financing costs + tax measures eat the operating gain"),

    ("Free cash flow", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Working capital reverses the record conversion"),

]
WEIGHTS = [0.20, 0.15, 0.15, 0.15, 0.20, 0.15]

STRUCTURAL_FACTORS = [
    ("Autoroute concession expiries (2031-2036) — the terminal-value overhang", -0.5, 0.25),
    ("~13.7x earnings with a 4.2% growing dividend — cheapest quality infra", 0.5, 0.2),
    ("Energies + Cobra: electrification backlog at records", 0.4, 0.2),
    ("Lower half of its 52-week range — no momentum premium to pay", 0.2, 0.15),
    ("French political/fiscal risk on tariffs and taxes", -0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 8.76   # FY2025 reported, EUR
EPP_MIN_PE       = 9.0    # concession floor — airport + autoroute cash flows through any cycle

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.0, "+2% revenue (vs +4%; macro drag)"),
    (1, 3.0, "Score 3/4 (vs 4; conversion slows)"),
    (2, 2.0, "Score 2/4 (vs 3; traffic flat)"),
    (3, 2.0, "Score 2/4 (vs 3; competition)"),
    (4, 2.0, "+2% net income (vs +5%)"),
    (5, 3.0, "Score 3/4 (vs 4)"),
]
CONS_EPS_CAGR = 0.05
CONS_EXIT_PE  = 12.0
CONS_DIVIDEND = 5.0
...
print()
print("═" * W)
print(f"  DG  ·  VINCI S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Concessions, Energy Solutions & Construction")
...
print(f"  H1 2026 (Jul 30): revenue EUR36.0bn, net income EUR2.08bn, record order book; interim dividend RAISED to EUR1.10")
...
print("""
  KEY TRIGGER: a French fiscal crisis producing BOTH a tax/tariff squeeze on
  the autoroutes AND a construction downturn — the cash machine and the volume
  machine hit by the same government. JOINT event: Airports + international
  Energies (60%+ of business outside France) cushion the home-market shock.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2025 reported, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [concession floor — airport + autoroute cash flows through any cycle]")
print(f"  {'─'*60}")
...
print("""
  At 5% EPS CAGR and a 12x exit, the 2-yr total return is modestly positive
  with the 4.2% dividend — an infra coupon priced without any growth credit.""")
...
print("""  Second-lowest volatility in the batch (16%, beta 0.85); the equity trades
  like the toll-road annuity it mostly is — the expiry debate caps the multiple.""")
