...
DBK Signal Model  v1
──────────────────────
Deutsche Bank AG (XETR: DBK)  ·  Universal Banking — CIB, Corporate, Private Bank, DWS
Trough year: 2019-20 (restructuring launch + COVID)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 33.06   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (2.4, 7, 17, "IB revenue normalizes hard; German credit turns; restructuring stalls"),
    "BASE": (3.4, 9, 30.5, "~EUR33bn revenue lands; ROTE grinds toward the >13% 2028 target"),
    "BULL": (3.9, 10, 39, "20-quarter growth streak extends; German fiscal stimulus lifts the home market"),
    "XBULL": (4.4, 11, 48, "Full re-rating to NW-European peer multiples as ROTE >13% proves durable"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E net revenues, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Investment Bank", "FIC + O&A — the swing factor; strongest and least stable line", 11.5, 10.0, 12.5),
    ("Private Bank", "German retail + international wealth — stable NII base", 9.6, 9.2, 10.0),
    ("Corporate Bank", "Payments/trade/lending — the quiet compounder", 7.8, 7.4, 8.2),
    ("Asset Mgmt (DWS)", "Fee stream with market beta", 2.8, 2.6, 3.1),
    ("Corporate & Other", "Funding costs + legacy runoff", 1.3, 0.8, 1.7),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Revenue growth", "% YoY",
      0.0, 3.0, 6.0, 9.0, 9.0, True,
     "IB normalization + NII fade flatten the topline"),

    ("Return on tangible equity", "%",
      6.0, 9.0, 11.5, 13.5, 11.9, True,
     "Cost or credit shock stalls the path to >13%"),

    ("Profit-growth streak & guidance", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "The 20-quarter streak breaks; guidance trimmed"),

    ("Cost/income ratio", "%",
      72.0, 67.0, 62.0, 58.0, 62.0, False,
     "Cost discipline slips against the <60% 2028 target"),

    ("CET1 ratio", "%",
      12.5, 13.2, 13.8, 14.4, 14.0, True,
     "Buybacks + RWA inflation erode the buffer"),

    ("IB dependence / revenue mix", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Markets revenue mean-reverts — the mix is the fragility"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Near its 52-week high after a multi-year re-rating run", -0.6, 0.25),
    ("~10x earnings — still a discount to NW-European peers", 0.4, 0.2),
    ("IB revenue volatility — the engine is the least repeatable line", -0.4, 0.2),
    ("2028 targets (ROTE >13%, C/I <60%) with visible momentum", 0.3, 0.2),
    ("German macro/credit exposure in a stagnant home economy", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 3.22   # TTM reported, EUR
EPP_MIN_PE       = 5.0    # restructured-bank trough floor — deposit + payments franchise

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 3.0, "+3% revenue (vs +9%; IB normalizes)"),
    (1, 10.5, "10.5% ROTE (vs 11.9)"),
    (2, 3.0, "Score 3/4 (vs 4; streak at risk)"),
    (3, 64.0, "64% C/I (vs 62)"),
    (4, 13.6, "13.6% CET1 (vs 14.0)"),
...
print()
print("═" * W)
print(f"  DBK  ·  Deutsche Bank AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Universal Banking — CIB, Corporate, Private Bank, DWS")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E net revenues, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026 (Jul 29): PBT EUR2.7bn +11%, 20th consecutive YoY profit-growth quarter; H1 ROTE 11.9%; FY26 revenue ~EUR33bn confirmed")
...
print("""
  KEY TRIGGER: an investment-banking revenue normalization landing WITH a
  German credit-cycle turn — the volatile engine and the stable base failing
  together. JOINT event: Corporate Bank + DWS fee streams cushion the mix.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [restructured-bank trough floor — deposit + payments franchise]")
print(f"  {'─'*60}")
...
print("""
  At 5% EPS CAGR and an 8x exit, the 2-yr return is mildly negative from near
  the high — the re-rating case now needs the 2028 targets, not just momentum.""")
...
print("""  Up ~39% from the 52-week low to within 3.5% of the high; beta 1.25 — this
  is still the highest-torque large European bank in either direction.""")
