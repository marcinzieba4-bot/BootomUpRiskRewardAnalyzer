...
DB1 Signal Model  v1
──────────────────────
Deutsche Börse AG (XETR: DB1)  ·  Exchange, Clearing, Fund Services & Market Data
Trough year: 2020 (COVID + low-rate treasury drag)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 258.1   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (11.0, 16, 176, "Volumes normalize + rate cuts crush treasury result; multiple derates"),
    "BASE": (13.5, 19, 257, "Guidance delivered: >EUR6.4bn revenue; secular + cyclical balance holds"),
    "BULL": (15.0, 21, 315, "Volatility regime + SimCorp/data compounding re-rate the mix"),
    "XBULL": (17.0, 23, 390, "Full data-and-software re-rating toward market-infrastructure peers"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E net revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Trading & Clearing", "Eurex derivatives + EEX power + Xetra — volatility is the raw material", 2.5, 2.3, 2.8),
    ("Fund Services & Custody", "Clearstream + IFS — asset-servicing annuity on EUR tn of assets", 2.0, 1.9, 2.1),
    ("Investment Mgmt Solutions", "SimCorp + ISS STOXX — software/data growth leg", 0.7, 0.65, 0.8),
    ("Treasury result", "NII on cash collateral — the rate-cycle windfall, now fading", 0.7, 0.5, 0.9),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Net revenue growth (ex-treasury)", "% YoY",
      0.0, 4.0, 7.0, 10.0, 9.0, True,
     "Volume normalization + fee pressure flatten growth"),

    ("EBITDA growth", "% YoY",
      0.0, 5.0, 9.0, 13.0, 13.0, True,
     "Cost growth outruns the operating leverage"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.5, True,
     "Outlook trimmed as treasury result fades"),

    ("Recurring-revenue mix", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Data/software growth stalls; cyclical volumes dominate again"),

    ("Capital returns", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Buyback pauses for M&A"),

    ("Volatility & rates environment", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Calm markets + fast cuts hit Eurex and treasury together"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("All-weather model: volatility hedge + structural data/software growth", 0.4, 0.2),
    ("Within ~8% of its 52-week high after a +18% YTD run", -0.4, 0.25),
    ("SimCorp/ISS STOXX shift toward recurring software revenue", 0.3, 0.2),
    ("~19x cash earnings — full for a partly cyclical mix", -0.3, 0.2),
    ("Treasury result is a fading rate-cycle windfall", -0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 13.0   # TTM cash EPS approx, EUR
EPP_MIN_PE       = 14.0    # market-infrastructure floor — clearing monopolies do not churn

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% revenue (vs +9%; volumes cool)"),
    (1, 7.0, "+7% EBITDA (vs +13%)"),
    (2, 3.0, "Score 3/4 (vs 3.5; held)"),
    (3, 3.0, "Score 3/4 (mix holds)"),
    (4, 2.0, "Score 2/4 (vs 3; M&A pause)"),
    (5, 2.0, "Score 2/4 (calm + cuts)"),
]
CONS_EPS_CAGR = 0.07
CONS_EXIT_PE  = 17.0
CONS_DIVIDEND = 4.2
...
print()
print("═" * W)
print(f"  DB1  ·  Deutsche Börse AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Exchange, Clearing, Fund Services & Market Data")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E net revenue, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026: net revenue ex-treasury +9% to EUR1.41bn, EBITDA +13%; H1 net profit +12%, cash EPS EUR6.73; FY outlook RAISED to >EUR6.4bn total")
...
print("""
  KEY TRIGGER: a calm-market regime arriving WITH fast ECB cuts — Eurex
  volumes and the treasury result (the two cyclical engines) fading together.
  JOINT event: Clearstream custody and SimCorp software are the acyclical keel.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (TTM cash EPS approx, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [market-infrastructure floor — clearing monopolies do not churn]")
print(f"  {'─'*60}")
...
print("""
  At 7% cash-EPS CAGR and a 17x exit, the 2-yr total return is roughly flat —
  quality infrastructure fully priced for its own reliability.""")
...
print("""  Lowest-risk profile in the batch (16% vol, beta 0.70) — the business is
  literally long volatility while the equity itself barely moves.""")
