...
ENI Signal Model  v1
──────────────────────
Eni S.p.A. (BIT: ENI)  ·  Integrated Oil & Gas · LNG · Energy Transition Satellites
Trough year: 2020 (oil crash)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 24.0   # EUR, Borsa Italiana, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (1.2, 13, 16, "Brent regime breaks lower; refining margins normalize; buyback throttled"),
    "BASE": (1.9, 13, 24.5, "CFFO ~EUR15bn holds; buyback EUR3.4bn executes; satellites monetize"),
    "BULL": (2.3, 13, 30, "Oil/LNG stays firm; extraordinary dividend lands; satellite IPOs re-rate"),
    "XBULL": (2.6, 13, 34, "Commodity supercycle + full satellite-model premium"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E pro forma EBIT, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Exploration & Production", "The engine — leveraged to Brent and gas realizations", 12.0, 8.0, 14.5),
    ("Global Gas & LNG", "Trading + LNG portfolio expanding through 2026-27 startups", 2.5, 1.8, 3.2),
    ("Enilive & Plenitude", "Biofuels + retail/renewables satellites — stake sales crystallize value", 2.2, 1.9, 2.6),
    ("Refining & Chemicals", "Margin-windfall beneficiary in 2026 — the least durable line", 1.3, 0.3, 2.0),
]
...
SIGNALS = [
    ("EBIT momentum", "/4",
     ...
     "Commodity realizations mean-revert; EBIT halves back"),

    ("CFFO guidance", "EURbn",
      11.0, 13.0, 14.5, 16.0, 15.0, True,
     "Guidance cut as refining/LNG margins normalize"),

    ("Buyback escalation", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Buyback throttled to defend the balance sheet"),

    ("Oil & gas price environment", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "OPEC+ supply + demand softness break the regime"),

    ("Satellite model (Enilive/Plenitude)", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Stake-sale valuations compress; IPO window closes"),

    ("Leverage & discipline", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Capex creep + payouts push gearing back up"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Within ~4% of its 52-week high after a +49% YTD run", -0.6, 0.25),
    ("Commodity-regime dependence — the P&L is a Brent/LNG call", -0.5, 0.25),
    ("Distribution escalation: buyback 2.3x initial plan + possible special dividend", 0.4, 0.2),
    ("Satellite model surfaces value the integrated multiple ignores", 0.3, 0.15),
    ("Refining-margin windfall is transient by construction", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 1.9   # FY2026E normalized (TTM EUR0.84 is trough-depressed), EUR
EPP_MIN_PE       = 8.0    # integrated-oil trough floor — the dividend franchise survives cycles

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 3.0, "Score 3/4 (vs 4; EBIT normalizes)"),
    (1, 13.5, "EUR13.5bn CFFO (vs 15)"),
    (2, 3.0, "Score 3/4 (vs 4; buyback trimmed)"),
    (3, 2.0, "Score 2/4 (vs 3; prices soften)"),
    (4, 2.0, "Score 2/4 (vs 3; window closes)"),
    (5, 3.0, "Score 3/4 (held)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 9.0
CONS_DIVIDEND = 1.1
...
print()
print("═" * W)
print(f"  ENI  ·  Eni S.p.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Integrated Oil & Gas · LNG · Energy Transition Satellites")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E pro forma EBIT, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026 (Jul 29): pro forma EBIT EUR5.4bn (2x YoY), net EUR2.3bn; CFFO guidance RAISED to EUR15bn; buyback RAISED to EUR3.4bn")
...
print("""
  KEY TRIGGER: a lower Brent regime arriving WITH refining-margin normalization
  — the windfall line and the core line deflating together, forcing the buyback
  down just as the market has priced its escalation. JOINT event: LNG startups
  and satellite disposals cushion the cash bridge.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2026E normalized (TTM EUR0.84 is trough-depressed), EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [integrated-oil trough floor — the dividend franchise survives cycles]")
print(f"  {'─'*60}")
...
print("""
  At 3% normalized-EPS CAGR and a 9x exit, the 2-yr return is meaningfully
  negative from a 52-week high — this is a commodity-regime bet, priced firm.""")
...
print("""  A +49% YTD run to within 4% of the high: energy equities bought after the
  commodity move, not before it, historically give back a third of the rally.""")
