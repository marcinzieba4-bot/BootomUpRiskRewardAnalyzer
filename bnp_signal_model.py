...
BNP Signal Model  v1
──────────────────────
BNP Paribas S.A. (EPA: BNP)  ·  Diversified European Banking & CIB
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 101.4   # EUR, Euronext Paris, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (10.0, 7, 70, "French politics + credit turn; markets revenue normalizes hard"),
    "BASE": (11.8, 8.5, 100, "Steady EPS grind; retail margin recovery offsets CIB normalization"),
    "BULL": (13.0, 9.4, 122, "CIB share gains stick; eurozone re-rating toward book value"),
    "XBULL": (15.0, 10.3, 155, "European banking-union premium; BNP re-rates as the continental champion"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Commercial & Personal Bkg", "French/Belgian/Italian retail — margins finally recovering as hedges roll", 27.5, 26.0, 29.0),
    ("Corporate & Inst. Banking", "Top-3 European CIB; Q2 equities was the standout engine", 18.5, 16.5, 20.5),
    ("Investment & Protection", "Insurance + asset management + wealth — the stable fee layer", 6.2, 5.8, 6.7),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("EPS growth trend", "% YoY",
      -5.0, 4.0, 9.0, 15.0, 9.0, True,
     "Provisions + French macro reverse the EPS trajectory"),

    ("CIB / equities momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Markets revenue mean-reverts after a record run"),

    ("Retail NII recovery", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "ECB cuts stall the French/Belgian margin rebuild"),

    ("CET1 ratio", "%",
      12.0, 12.6, 13.0, 13.5, 13.0, True,
     "Capital consumed by RWA inflation + buybacks"),

    ("Cost/income ratio", "%",
      68.0, 63.0, 60.0, 57.0, 61.0, False,
     "Cost discipline slips vs the 2026 plan"),

    ("Capital distributions", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Payout throttled by supervisory or political pressure"),

]
WEIGHTS = [0.20, 0.15, 0.15, 0.15, 0.15, 0.20]

STRUCTURAL_FACTORS = [
    ("Scale + diversification — most balanced European bank model", 0.4, 0.2),
    ("~9x earnings, still a discount to NW-European peers", 0.5, 0.2),
    ("Markets-revenue dependence — Q2's engine is the least repeatable line", -0.4, 0.2),
    ("+56% off the 52-week low; the easy re-rating is done", -0.5, 0.25),
    ("French political/fiscal overhang on the home market", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 11.55   # TTM reported, EUR
EPP_MIN_PE       = 6.0    # French-bank trough floor — deposit franchise + CIB book survive cycles

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 4.0, "+4% EPS (vs +9%; provisions normalize)"),
    (1, 2.0, "Score 2/4 (vs 4; markets mean-revert)"),
    (2, 2.0, "Score 2/4 (vs 3; ECB cuts bite)"),
    (3, 12.7, "12.7% CET1 (vs 13.0)"),
    (4, 62.5, "62.5% C/I (vs 61; cost creep)"),
    (5, 2.0, "Score 2/4 (vs 3; payout capped)"),
]
CONS_EPS_CAGR = 0.04
CONS_EXIT_PE  = 8.0
CONS_DIVIDEND = 5.57
...
print()
print("═" * W)
print(f"  BNP  ·  BNP Paribas S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Diversified European Banking & CIB")
...
print(f"  Q2 2026: operating profit +16% on record equities trading + French/Belgian retail margin recovery")
...
print("""
  KEY TRIGGER: a French political/fiscal crisis re-pricing the home franchise
  WHILE markets revenue normalizes off its record — the cyclical engine and the
  domestic anchor failing together. JOINT event: diversification cushions one.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [French-bank trough floor — deposit franchise + CIB book survive cycles]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and an 8x exit, the 2-yr price return is negative; the 5.5%
  distribution stream does the heavy lifting in the hold case.""")
...
print("""  Up ~56% from the 52-week low (EUR65.12) though still ~11% below the high;
  the re-rating from 6x to 9x is done — from here earnings must do the work.""")
