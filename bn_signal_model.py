...
BN Signal Model  v1
──────────────────────
Danone S.A. (EPA: BN)  ·  Dairy, Plant-Based, Specialized Nutrition & Waters
Trough year: 2022 (inflation squeeze + strategy reset)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 65.04   # EUR, Euronext Paris, 2026-08-27
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (3.3, 15, 50, "Volume/mix stalls; China nutrition weakens; margin grind stops"),
    "BASE": (4.0, 17, 68, "3-5% LFL delivered with margin expansion; portfolio compounds quietly"),
    "BULL": (4.4, 19, 84, "Medical nutrition + mix accelerate; multiple re-rates toward staples peers"),
    "XBULL": (4.9, 21, 103, "Full portfolio premiumization — health-science multiple achieved"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Essential Dairy & Plant-Based", "Activia/Alpro/Danone core — steady, low growth, improving mix", 13.5, 13.0, 14.0),
    ("Specialized Nutrition", "Infant formula + MEDICAL nutrition — the margin and growth engine", 9.3, 8.9, 9.8),
    ("Waters", "evian/Volvic/Mizone — volume recovering, China-sensitive", 5.3, 5.0, 5.7),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Like-for-like growth", "% YoY",
      0.0, 2.0, 4.0, 6.0, 4.2, True,
     "Volume/mix stalls as pricing laps out"),

    ("Margin trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Input costs + reinvestment stop the +bp grind"),

    ("Volume/mix quality", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Growth reverts to price-only — the low-quality kind"),

    ("Specialized Nutrition momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "China birth-rate drag + competition in medical nutrition"),

    ("Guidance delivery", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "3-5% LFL corridor abandoned"),

    ("China & EM dynamics", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Mizone/formula demand rolls over on macro weakness"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~19% below its 52-week high — rerating risk already partly released", 0.3, 0.2),
    ("Beta 0.55 defensive staple with accelerating LFL — rare combination", 0.3, 0.2),
    ("Category growth ceiling: dairy/waters cap the upside arithmetic", -0.4, 0.25),
    ("~16.5x recurring EPS — undemanding vs staples peers", 0.2, 0.2),
    ("EM/China exposure in nutrition and waters", -0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 3.9   # FY2026E recurring approx, EUR
EPP_MIN_PE       = 13.0    # staples floor — infant/medical nutrition is need-based demand

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.5, "+2.5% LFL (vs +4.2%; pricing laps)"),
    (1, 2.0, "Score 2/4 (vs 3; grind pauses)"),
    (2, 2.0, "Score 2/4 (vs 3; price-led)"),
    (3, 2.0, "Score 2/4 (vs 3; China drag)"),
    (4, 2.0, "Score 2/4 (corridor floor)"),
...
print()
print("═" * W)
print(f"  BN  ·  Danone S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Dairy, Plant-Based, Specialized Nutrition & Waters")
...
print(f"  H1 2026 (Jul 29): sales EUR13.9bn, LFL +3.5% with Q2 ACCELERATING to +4.2%; recurring margin 13.3% (+12bp); FY guidance confirmed 3-5% LFL")
...
print("""
  KEY TRIGGER: a China demand shock hitting Specialized Nutrition and Waters
  SIMULTANEOUSLY while European dairy volumes stall — the growth engine and
  the base failing together. JOINT event: medical nutrition (hospital-channel,
  need-based) is the resilient core inside the growth segment.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [staples floor — infant/medical nutrition is need-based demand]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and a 15x exit, the 2-yr total return is modestly positive
  with the 3.4% dividend — a defensive coupon with self-help upside.""")
...
print("""  The lowest-beta name in the batch (0.55, 15% vol): the market prices Danone
  as a bond-plus. The Q2 acceleration is what a turn looks like if it holds.""")
