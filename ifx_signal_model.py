...
IFX Signal Model  v1
──────────────────────
Infineon Technologies AG (XETR: IFX)  ·  Power Semiconductors — Auto, AI Datacenter, Industrial
Trough year: 2020 (COVID auto shutdown + Cypress digestion)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 55.97   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (1.0, 22, 22, "Auto semi downcycle deepens; AI datacenter capex pauses; margin resets"),
    "BASE": (1.9, 28, 53, "FY26 ~EUR16.3bn lands; AI power ramps; auto bottoms and recovers"),
    "BULL": (2.4, 30, 72, "AI power-delivery share gains compound; margin >23% sustained"),
    "XBULL": (2.9, 31, 90, "Full AI-infrastructure supercycle; IFX re-rates as the power pure-play"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Automotive", "~half of revenue — EV/ADAS content growth vs inventory cycle", 8.2, 7.4, 8.8),
    ("Power & Sensor Systems", "AI datacenter power delivery — the growth engine, record demand", 3.6, 3.2, 4.2),
    ("Green Industrial Power", "Grid, renewables, industrial drives — structural electrification", 2.6, 2.3, 3.0),
    ("Connected Secure Systems", "IoT + security MCUs — steady, less cyclical", 1.9, 1.7, 2.1),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Revenue growth", "% YoY",
      0.0, 5.0, 10.0, 15.0, 13.0, True,
     "AI orders digest; auto inventory correction resumes"),

    ("Segment margin", "%",
      12.0, 16.0, 20.0, 24.0, 19.1, True,
     "Utilization drops; pricing normalizes off the AI premium"),

    ("AI / datacenter power momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Hyperscaler capex pause hits the order book"),

    ("Automotive semis cycle", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "EV slowdown + China competition squeeze the core"),

    ("FY guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.5, True,
     "Guidance cut on FX or demand"),

    ("Adjusted free cash flow", "EURbn",
      0.8, 1.3, 1.8, 2.3, 1.85, True,
     "Capex absorbs the cash as fabs expand"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("AI power-delivery leadership — structural demand from every datacenter", 0.6, 0.25),
    ("Semiconductor cyclicality — margins and multiples reset violently", -0.4, 0.2),
    ("~37% below its 52-week high after the sector correction", 0.3, 0.15),
    ("Automotive concentration (~half of revenue) in a soft EV cycle", -0.4, 0.2),
    ("Record quarter + raised outlook — momentum validated", 0.2, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 1.55   # TTM approx, EUR
EPP_MIN_PE       = 14.0    # semi-trough floor — power franchise survives the downcycle

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 7.0, "+7% revenue (vs +13%; digestion)"),
    (1, 17.0, "17% margin (vs 19.1%)"),
    (2, 3.0, "Score 3/4 (vs 4; capex pauses)"),
    (3, 2.0, "Score 2/4 (vs 2.5; auto soft)"),
    (4, 3.0, "Score 3/4 (vs 3.5; held not raised)"),
    (5, 1.5, "EUR1.5bn FCF (vs 1.85)"),
]
CONS_EPS_CAGR = 0.1
CONS_EXIT_PE  = 24.0
CONS_DIVIDEND = 0.35
...
print()
print("═" * W)
print(f"  IFX  ·  Infineon Technologies AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Power Semiconductors — Auto, AI Datacenter, Industrial")
...
print(f"  Q3 FY2026 (Aug 5): record revenue EUR4.17bn +13%, all divisions growing; Q4 guided EUR4.7bn at ~23% segment margin")
...
print("""
  KEY TRIGGER: an AI-datacenter capex digestion phase arriving WHILE the auto
  semiconductor inventory correction is still running — both demand engines
  pausing at once. JOINT event: industrial/grid demand cushions the middle.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [semi-trough floor — power franchise survives the downcycle]")
print(f"  {'─'*60}")
...
print("""
  At 10% EPS CAGR and a 24x exit, the 2-yr return is negative from EUR56 —
  the AI story is real, but semis never let you pay mid-cycle prices safely.""")
...
print("""  Highest-volatility addition (42%, beta 1.40): a EUR30.82-88.83 52-week range
  tells you exactly what owning a power-semi pure-play through a hype cycle costs.""")
