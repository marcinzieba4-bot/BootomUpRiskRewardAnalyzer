...
ALV Signal Model  v1
──────────────────────
Allianz SE (XETR: ALV)  ·  Global Insurance & Asset Management
Trough year: 2022 (Structured Alpha settlement + rate shock)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 450.0   # EUR, Xetra, 2026-08-25
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (25.0, 12, 300, "Major cat year + PIMCO outflows + solvency drawdown"),
    "BASE": (32.0, 14, 450, "EUR17.4bn OP guidance delivered; steady buybacks/dividend"),
    "BULL": (36.0, 15.5, 560, "Record OP momentum extends; AM inflows persist; re-rating"),
    "XBULL": (40.0, 16.5, 660, "Multi-year hard P&C market + PIMCO supercycle + capital returns"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E operating profit, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Property & Casualty", "CR 91.4% in Q2, volume ~EUR50bn +6% internal — the profit engine", 8.2, 7.2, 8.8),
    ("Life & Health", "VONB EUR2.4bn +4%; stable margin annuity/protection book", 5.4, 5.0, 5.8),
    ("Asset Management", "PIMCO + AGI: record EUR84bn H1 third-party inflows, AuM EUR2.16tn", 3.8, 3.2, 4.2),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Operating profit growth YoY", "% YoY",
      -5.0, 3.0, 8.0, 12.0, 9.0, True,
     "Nat-cat losses + rate pressure stall OP growth"),

    ("P&C combined ratio", "%",
      97.0, 94.0, 92.0, 90.0, 91.4, False,
     "Cat season + claims inflation push CR toward 97%"),

    ("Solvency II ratio", "%",
      180.0, 200.0, 215.0, 230.0, 225.0, True,
     "Market shock + credit losses erode capital buffer"),

    ("AM third-party net inflows", "EURbn",
      -20.0, 20.0, 60.0, 100.0, 84.0, True,
     "Rate volatility triggers PIMCO outflows again"),

    ("L&H new business value growth", "% YoY",
      -5.0, 0.0, 4.0, 8.0, 4.0, True,
     "New business margin compresses"),

    ("FY guidance confidence", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Guidance cut below the EUR17.4bn midpoint"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.10, 0.20]

STRUCTURAL_FACTORS = [
    ("Record solvency (225%) + record H1 operating profit", 0.7, 0.25),
    ("Diversified three-pillar model smooths single-segment shocks", 0.5, 0.2),
    ("~15x trailing EPS — undemanding for record-profitability insurer", 0.4, 0.2),
    ("Nat-cat / climate loss trend structurally rising", -0.5, 0.2),
    ("PIMCO strategic-review headlines create uncertainty overhang", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 30.49   # TTM reported, EUR
EPP_MIN_PE       = 9.0    # insurer trough floor — fortress solvency, diversified book

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 4.0, "+4% OP (vs +9%; normalization)"),
    (1, 93.5, "93.5% CR (vs 91.4%; avg cat year)"),
    (2, 210.0, "210% solvency (vs 225%; buybacks)"),
    (3, 30.0, "EUR30bn inflows (vs 84; normalize)"),
    (4, 1.0, "+1% VONB (vs +4%; margin pressure)"),
    (5, 2.0, "Score 2/4 (vs 3; mid-guidance)"),
]
CONS_EPS_CAGR = 0.05
CONS_EXIT_PE  = 12.0
CONS_DIVIDEND = 17.1
...
print()
print("═" * W)
print(f"  ALV  ·  Allianz SE  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Global Insurance & Asset Management")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E operating profit, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026 actual: record OP EUR4.9bn (+10.6%); H1 EUR9.4bn (+9%); FY guidance EUR17.4bn +/-1bn confirmed")
...
print("""
  KEY TRIGGER: a major catastrophe year (CR toward 97%) coinciding with a rate
  shock that reopens PIMCO outflows — P&C and AM stumbling together. JOINT
  event: 225% solvency absorbs either alone; the pair pressures the dividend story.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [insurer trough floor — fortress solvency, diversified book]")
print(f"  {'─'*60}")
...
print("""
  At a conservative 5% EPS CAGR and 12x exit, price return is modestly positive
  and the ~3.8% dividend yield does the rest — a fortress-balance-sheet compounder.""")
...
print("""  Lowest volatility (18%) of the European additions — but the stock sits within
  1% of its 52-week high, so entry timing matters more than usual for an insurer.""")
