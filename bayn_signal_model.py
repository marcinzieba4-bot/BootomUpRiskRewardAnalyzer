#!/usr/bin/env python3
"""
BAYN Signal Model  v1
──────────────────────
Bayer AG (XETR: BAYN)  ·  Pharma · Crop Science · Consumer Health
Trough year: 2024 (litigation despair low EUR25.78)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 49.26   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (3.0, 7, 21, "Settlement collapses on opt-outs; pharma cliff bites; dividend stays token"),
    "BASE": (4.8, 10, 48, "Glyphosate settlement approved; deleveraging continues; core EPS stabilizes"),
    "BULL": (5.5, 12, 66, "Litigation closed + pipeline (elinzanetant, acoramidis) offsets the cliff"),
    "XBULL": (6.5, 14, 91, "Clean Bayer re-rates; conglomerate discount unwinds toward sum-of-parts"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Crop Science", "World #1 seeds/crop protection — the earnings beat driver in 2026", 23.0, 21.5, 24.0),
    ("Pharmaceuticals", "Xarelto/Eylea cliffs vs Nubeqa/Kerendia/elinzanetant ramp", 17.5, 16.5, 18.5),
    ("Consumer Health", "OTC brands — steady, cash-generative, non-controversial", 6.0, 5.7, 6.3),
]
...
SIGNALS = [
    ("Operational momentum", "/4",
     ...
     "Crop cycle turns; pharma erosion accelerates"),

    ("Litigation resolution progress", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Opt-out wave sinks the Sep 10 settlement approval"),

    ("Net-debt trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Litigation cash outflows reverse the deleveraging"),

    ("Crop Science momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Ag commodity downturn + generic dicamba pressure"),

    ("Pharma pipeline vs patent cliff", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Nubeqa/Kerendia ramp too slow for the Xarelto/Eylea hole"),

    ("Core EPS trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Cliffs + litigation costs keep core EPS flat-to-down"),

]
WEIGHTS = [0.15, 0.25, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Glyphosate settlement approval (Sep 10) — the binary that unlocks the story", 0.4, 0.2),
    ("+32% YTD, near its 52-week high — recovery partly pre-paid", -0.5, 0.25),
    ("Pharma patent cliff (Xarelto, Eylea) still unsolved by the pipeline", -0.5, 0.2),
    ("Net debt EUR29-30bn — better, still heavy vs ~EUR8bn EBITDA", -0.3, 0.15),
    ("Sum-of-parts optionality: three separable businesses under one discount", 0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 4.8   # FY2026E core EPS approx, EUR
EPP_MIN_PE       = 6.0    # litigation-discount floor — Crop Science alone underpins it

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.0, "Score 2/4 (vs 3; momentum fades)"),
    (1, 2.0, "Score 2/4 (approval delayed again)"),
    (2, 3.0, "Score 3/4 (vs 4; outflows resume)"),
    (3, 2.0, "Score 2/4 (ag cycle turns)"),
    (4, 2.0, "Score 2/4 (cliff unsolved)"),
    (5, 2.0, "Score 2/4 (EPS flat)"),
]
CONS_EPS_CAGR = 0.05
CONS_EXIT_PE  = 8.0
CONS_DIVIDEND = 0.11
...
print()
print("═" * W)
print(f"  BAYN  ·  Bayer AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Pharma · Crop Science · Consumer Health")
...
print(f"  Q2 2026 (Aug 4): beat estimates (revenue EUR12.52bn); net-debt target IMPROVED to EUR29-30bn; glyphosate class settlement ($7.25bn) final hearing Sep 10")
...
print("""
  KEY TRIGGER: the Sep 10 settlement hearing failing on opt-outs WHILE the
  pharma cliff revenue hole widens — the legal overhang returning just as the
  operating engine weakens. JOINT event: Crop Science and Consumer Health cash
  flows are unaffected by either.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2026E core EPS approx, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [litigation-discount floor — Crop Science alone underpins it]")
print(f"  {'─'*60}")
...
print("""
  At 5% core-EPS CAGR and an 8x exit, the 2-yr return is negative from EUR49 —
  after a +32% YTD run the easy litigation-relief money has been made.""")
...
print("""  A EUR25.78-53.92 52-week range tells the story: this equity has repriced
  from despair to hope in twelve months. Sep 10 is the next binary.""")
