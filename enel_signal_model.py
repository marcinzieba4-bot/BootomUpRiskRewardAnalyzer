#!/usr/bin/env python3
"""
ENEL Signal Model  v1
──────────────────────
Enel S.p.A. (BIT: ENEL)  ·  Integrated Utility — Networks & Renewables
Trough year: 2022 (rate shock + leverage fears)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 9.49   # EUR, Borsa Italiana, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (0.65, 11, 7.2, "Rates re-spike; Latin America FX + regulatory resets bite"),
    "BASE": (0.78, 12, 9.4, "EPS guidance EUR0.72-0.76 lands; grids capex compounds the RAB"),
    "BULL": (0.85, 13, 11.0, "Rate cuts + grid supercycle re-rate the regulated base"),
    "XBULL": (0.93, 14, 13.0, "Full utility re-rating as electrification demand outruns supply"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E ordinary EBITDA, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Grids (regulated)", "Italy/Iberia/LatAm distribution — the visibility engine, tariff-indexed", 9.2, 8.8, 9.7),
    ("Green Power & Thermal", "Renewables fleet + flexible backup; captured-price sensitive", 8.3, 7.5, 9.0),
    ("Markets (retail)", "Italian + Iberian customer base; margin recovered post-crisis", 4.6, 4.2, 5.0),
    ("Other / holdings", "Services, corporate center, minorities", 0.9, 0.8, 1.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Ordinary net income growth", "% YoY",
      -3.0, 2.0, 6.0, 10.0, 3.0, True,
     "Regulatory resets + FX reverse the grind higher"),

    ("Grids EBITDA momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Tariff reviews turn hostile in Italy/LatAm"),

    ("Renewables output & pricing", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Hydro/wind resource + captured prices disappoint"),

    ("Net debt / EBITDA", "x",
      3.2, 2.9, 2.7, 2.5, 2.8, False,
     "Capex + payouts push leverage back up"),

    ("Dividend floor coverage", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Cash conversion stops covering the EUR0.49 floor"),

    ("Interest-rate environment", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Rates re-spike; utility discount rates blow out"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Grid-capex visibility — regulated RAB growth tariff-indexed", 0.5, 0.25),
    ("5.2% dividend floor (EUR0.49 minimum) — committed, covered", 0.4, 0.2),
    ("Low structural growth ceiling — a 15%/yr hurdle is alien territory", -0.4, 0.2),
    ("Latin America FX translation permanently noisy", -0.3, 0.2),
    ("Rate sensitivity of both valuation and funding", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 0.74   # FY2026E guidance midpoint, EUR
EPP_MIN_PE       = 9.0    # regulated-utility floor — grids RAB underpins

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 1.5, "+1.5% income (vs +3%; FX drag)"),
    (1, 2.0, "Score 2/4 (vs 3; resets bite)"),
    (2, 2.0, "Score 2/4 (vs 2.5)"),
    (3, 2.9, "2.9x leverage (vs 2.8)"),
    (4, 3.0, "Floor held (unchanged)"),
    (5, 2.0, "Score 2/4 (vs 2.5; rates sticky)"),
]
CONS_EPS_CAGR = 0.04
CONS_EXIT_PE  = 11.0
CONS_DIVIDEND = 0.49
...
print()
print("═" * W)
print(f"  ENEL  ·  Enel S.p.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Integrated Utility — Networks & Renewables")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E ordinary EBITDA, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  H1 2026: EBITDA EUR11.8bn +3%; net ordinary income EUR3.9bn +3%; FY EPS guided EUR0.72-0.76; dividend floor EUR0.49")
...
print("""
  KEY TRIGGER: a rates re-spike hitting the utility discount rate WHILE Italian
  or Latin American regulators turn hostile on tariff resets — multiple and
  earnings hit at once. JOINT event: the dividend floor cushions the equity.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2026E guidance midpoint, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [regulated-utility floor — grids RAB underpins]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and an 11x exit, the 2-yr price return is negative and the
  EUR0.49 floor dividend (~10.3% over 2yr) roughly closes the gap — bond-like.""")
...
print("""  A 5.2% committed dividend floor with 18% vol: the equity is priced as a
  rate instrument, and it will trade like one in both directions.""")
