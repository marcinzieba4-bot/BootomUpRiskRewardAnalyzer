...
AIR Signal Model  v1
──────────────────────
Airbus SE (EPA: AIR)  ·  Commercial Aerospace & Defence
Trough year: 2020 (COVID air-travel collapse)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 202.7   # EUR, Euronext Paris, 2026-08-24
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (4.5, 30, 135, "Engine/supply chain caps deliveries; FCF stays negative; guidance cut"),
    "BASE": (7.5, 28, 210, "~870 deliveries delivered; EBIT ~EUR7.5bn; FCF EUR4.5bn lands"),
    "BULL": (9.5, 27, 260, "Rate-75 ramp on schedule; margin expansion; defence momentum compounds"),
    "XBULL": (11.5, 27, 310, "Supply chain fully heals + defence rearmament supercycle + Boeing share gains"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Commercial Aircraft", "H1: 351 deliveries (+15%), revenue EUR23.9bn (+15%); 9,222-aircraft backlog", 55.0, 48.0, 60.0),
    ("Defence & Space", "European rearmament tailwind; gaining momentum after restructuring", 13.5, 12.5, 15.0),
    ("Helicopters", "Steady civil + military demand; least cyclical division", 8.0, 7.5, 8.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Deliveries growth (H1)", "% YoY",
      0.0, 8.0, 15.0, 22.0, 15.0, True,
     "Engine and interiors shortages cap output again"),

    ("FY delivery guidance", "aircraft",
      780.0, 830.0, 870.0, 900.0, 870.0, True,
     "Guidance cut on supplier shortfalls"),

    ("Adj EBIT margin", "%",
      5.0, 7.0, 9.0, 11.0, 8.3, True,
     "Ramp-up costs + tariffs squeeze margins"),

    ("Order backlog", "aircraft",
      8000.0, 8800.0, 9200.0, 9600.0, 9222.0, True,
     "Cancellations/deferrals dent the record backlog"),

    ("Free cash flow trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Working-capital drag persists; FCF guidance missed"),

    ("A320 rate-75 ramp on track", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Rate 75 pushed beyond end-2027"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.10, 0.20, 0.15]

STRUCTURAL_FACTORS = [
    ("Duopoly + 9,222-aircraft backlog — roughly a decade of production", 1.0, 0.3),
    ("H1 free cash flow negative (-EUR1.2bn) on inventory build", -0.5, 0.2),
    ("Supply-chain fragility (engines) remains the binding constraint", -0.5, 0.2),
    ("Defence & Space momentum from European rearmament", 0.5, 0.15),
    ("Tariff exposure on US-bound deliveries", -0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 7.52   # TTM reported, EUR
EPP_MIN_PE       = 15.0    # duopoly floor — decade-long backlog de-risks demand entirely

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 8.0, "+8% deliveries (vs +15%; supply caps)"),
    (1, 840.0, "840 aircraft (vs 870; slippage)"),
    (2, 7.5, "7.5% margin (vs 8.3%; ramp costs)"),
    (3, 9000.0, "9,000 backlog (vs 9,222)"),
    (4, 2.0, "Score 2/4 (vs 2; drag persists)"),
    (5, 2.0, "Score 2/4 (vs 3; ramp slips a bit)"),
]
CONS_EPS_CAGR = 0.1
CONS_EXIT_PE  = 20.0
CONS_DIVIDEND = 3.2
...
print()
print("═" * W)
print(f"  AIR  ·  Airbus SE  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Commercial Aerospace & Defence")
...
print(f"  H1 2026 actual: adj EBIT +16%, FY guidance held: ~870 deliveries, ~EUR7.5bn EBIT, ~EUR4.5bn FCF")
...
print("""
  KEY TRIGGER: engine-maker output shortfalls capping deliveries below 800 while
  inventory build keeps FCF negative into 2027 — a cash-conversion crisis inside
  a demand boom. JOINT event: demand itself is de-risked by the decade-long backlog.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [duopoly floor — decade-long backlog de-risks demand entirely]")
print(f"  {'─'*60}")
...
print("""
  At 10% EPS CAGR and a 20x exit, the 2-yr return is slightly negative from
  today's price — the duopoly quality is real, but so is the execution premium.""")
...
print("""  The bear case here is purely operational (supply chain, cash), never demand:
  9,222 aircraft of backlog means Airbus sells everything it can build for a decade.""")
