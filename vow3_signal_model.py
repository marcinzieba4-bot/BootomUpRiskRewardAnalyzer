#!/usr/bin/env python3
"""
VOW3 Signal Model  v1
──────────────────────
Volkswagen AG (Pref) (XETR: VOW3)  ·  Mass-Market to Luxury Automobiles — 10 Brands
Trough year: 2020 (COVID production halt)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 74.84   # EUR, Xetra preference shares, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (12.0, 4.5, 54, "China collapse + tariffs + EV price war; restructuring stalls against the unions"),
    "BASE": (17.0, 5.0, 85, "Margin 4-5.5% held; Target Picture 2030 cuts costs; China stabilizes low"),
    "BULL": (20.0, 5.8, 116, "Restructuring delivers; Porsche/Audi recover; tariffs ease"),
    "XBULL": (24.0, 6.5, 156, "Full turnaround + software/EV credibility restored — the deep-value dream"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Core brands (VW, Skoda, SEAT)", "Volume engine — margin-thin, restructuring focus", 145.0, 135.0, 150.0),
    ("Progressive (Audi, Bentley, Lambo)", "Premium arm — Audi's China/product malaise is the drag", 70.0, 64.0, 74.0),
    ("Sport Luxury (Porsche AG)", "The crown jewel — its own China/EV reset underway", 38.0, 35.0, 41.0),
    ("CARIAD & Battery", "Software + PowerCo cells — cash sink, strategic necessity", 3.0, 2.0, 4.0),
    ("Financial Services", "Captive finance — stable earnings ballast", 62.0, 60.0, 64.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Global deliveries trend", "% YoY",
      -8.0, -3.0, 1.0, 5.0, -6.3, True,
     "Volume decline broadens across regions"),

    ("China performance", "/4",
      1.0, 2.0, 3.0, 4.0, 1.0, True,
     "Local EV competition takes share across all group brands"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Margin corridor cut next after the revenue cut"),

    ("Operating margin corridor", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Tariffs + mix push RoS below 4%"),

    ("Restructuring (Target Picture 2030)", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Union resistance dilutes the cost program"),

    ("Tariff & trade exposure", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "US/EU trade regime worsens beyond EUR5bn/yr"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~4.4x earnings + 7.0% dividend — among the cheapest large caps in Europe", 0.4, 0.2),
    ("China structural share loss across VW, Audi and Porsche at once", -0.6, 0.25),
    ("EUR4-5bn annual tariff burden with no negotiating leverage", -0.4, 0.2),
    ("Within ~8% of its 52-week low — expectations at rock bottom", 0.3, 0.15),
    ("Software (CARIAD) and EV execution track record remains poor", -0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 17.0   # TTM approx, EUR
EPP_MIN_PE       = 3.5    # deep-cyclical trough floor — Porsche stake + FS book underpin

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, -4.0, "-4% deliveries (vs -6.3; slow fade)"),
    (1, 1.0, "Score 1/4 (China stays lost)"),
    (2, 2.0, "Score 2/4 (held down)"),
    (3, 2.0, "Score 2/4 (vs 2.5; low end)"),
    (4, 2.0, "Score 2/4 (vs 3; slower cuts)"),
    (5, 2.0, "Score 2/4 (regime persists)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 4.0
CONS_DIVIDEND = 5.26
...
print()
print("═" * W)
print(f"  VOW3  ·  Volkswagen AG (Pref)  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Mass-Market to Luxury Automobiles — 10 Brands")
...
print(f"  H1 2026 (Jul 24): revenue EUR158.1bn flat, op profit -12% to EUR5.9bn; deliveries -6.3%; revenue guidance CUT to -3%-to-flat; margin 4-5.5% held; tariffs a EUR4-5bn/yr burden")
...
print("""
  KEY TRIGGER: already partially live — China share loss and the tariff regime
  ARE the bear case running. Full BEAR needs Europe/Americas volumes to crack
  too while the union blocks Target Picture 2030 cost cuts. JOINT event:
  Financial Services and the Porsche stake are the non-cyclical ballast.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [deep-cyclical trough floor — Porsche stake + FS book underpin]")
print(f"  {'─'*60}")
...
print("""
  At 3% EPS CAGR and a 4x exit, the 2-yr total return is ~+12% mostly from the
  7% dividend — the equity is an option on restructuring, paid to wait.""")
...
print("""  A EUR69-109 52-week range with the price pinned near the floor: the market
  has stopped pricing a turnaround and started pricing endurance.""")
