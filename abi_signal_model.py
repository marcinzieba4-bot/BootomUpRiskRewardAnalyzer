...
ABI Signal Model  v1
──────────────────────
Anheuser-Busch InBev SA/NV (EBR: ABI)  ·  Global Beer & Beverages
Trough year: 2020 (COVID on-trade collapse + leverage fears)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 67.72   # EUR, Euronext Brussels, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (3.0, 17, 50, "Volume decline accelerates; EM FX crush; deleveraging stalls"),
    "BASE": (4.3, 16, 68, "Low-single-digit organic growth; premiumization mix; debt paydown continues"),
    "BULL": (5.0, 16, 82, "Volume inflection + margin expansion + buybacks scale up"),
    "XBULL": (5.8, 16, 95, "EM beer supercycle + full re-rating toward staples multiples"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn approx (reports in USD)) ────────────────────────────────
SEGMENTS = [
    ("Middle Americas", "Mexico/Colombia — largest and most profitable region", 13.8, 12.8, 14.6),
    ("North America", "US Bud Light recovery ongoing; share stabilizing", 13.4, 12.6, 14.0),
    ("South America", "Brazil-led; volume engine with FX translation noise", 10.8, 9.8, 11.8),
    ("EMEA", "Europe premiumization + Africa growth (Castel stake)", 8.2, 7.6, 8.8),
    ("Asia-Pacific", "China premium + India buildout — long-duration option", 7.3, 6.6, 8.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Organic revenue growth", "% YoY",
      -2.0, 2.0, 5.0, 8.0, 5.0, True,
     "Volume declines outpace pricing; organic growth turns negative"),

    ("Organic EBITDA growth", "% YoY",
      0.0, 4.0, 8.0, 12.0, 8.0, True,
     "Cost inflation + mix erode margin leverage"),

    ("Volume trend", "% YoY",
      -3.0, 0.0, 1.5, 3.0, 1.0, True,
     "Global beer volumes resume secular decline"),

    ("Net debt / EBITDA", "x",
      4.0, 3.3, 3.0, 2.6, 2.9, False,
     "FX-inflated debt stalls deleveraging below 3x"),

    ("Premiumization / share gains", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Premium mix stalls; craft/spirits substitution"),

    ("Free cash flow strength", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Capex + working capital absorb the FCF story"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Unmatched global scale + #1 positions across EM beer", 0.6, 0.25),
    ("Deleveraging below 3x unlocks buyback capacity", 0.4, 0.2),
    ("Secular beer-volume stagnation in developed markets", -0.5, 0.25),
    ("EM FX translation permanently noisy (BRL, MXN, ARS)", -0.3, 0.15),
    ("~16x trailing — reasonable vs staples peers", 0.3, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 4.13   # TTM reported, EUR
EPP_MIN_PE       = 12.0    # staples-trough floor — brand portfolio + EM distribution moat

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 2.5, "+2.5% organic (vs +5%; volumes soft)"),
    (1, 5.0, "+5% EBITDA (vs +8%)"),
    (2, 0.0, "flat volumes (vs +1%)"),
    (3, 3.1, "3.1x leverage (vs 2.9; FX)"),
    (4, 2.0, "Score 2/4 (vs 3; mix stalls)"),
...
print()
print("═" * W)
print(f"  ABI  ·  Anheuser-Busch InBev SA/NV  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Global Beer & Beverages")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E revenue, EUR bn approx (reports in USD)  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026 (Jul 30): revenue and EPS growth with share gains in key regions and robust free cash flow")
...
print("""
  KEY TRIGGER: developed-market volume decline accelerating WHILE EM currencies
  (BRL/MXN) devalue — organic growth and translation failing together, stalling
  deleveraging. JOINT event: geographic breadth cushions any single region.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [staples-trough floor — brand portfolio + EM distribution moat]")
print(f"  {'─'*60}")
...
print("""
  At 5% EPS CAGR and a 14x exit, the 2-yr return is modestly negative — the
  deleveraging-to-buyback pivot is the bull case, and it needs FX cooperation.""")
...
print("""  Middle of its 52-week range (EUR48.87-76.18) — unusually, NOT at a high, which
  makes this the least momentum-dependent entry among the new additions.""")
