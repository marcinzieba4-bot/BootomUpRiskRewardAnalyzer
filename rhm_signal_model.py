...
RHM Signal Model  v1
──────────────────────
Rheinmetall AG (XETR: RHM)  ·  Defense — Vehicles, Ammunition, Electronics
Trough year: 2020 (pre-war defense trough)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 1122.0   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (25.0, 24, 600, "Ceasefire re-rating; German orders slip; margin ramp stalls"),
    "BASE": (31.0, 36, 1120, "EUR13.7-14.2bn revenue lands; backlog converts on schedule"),
    "BULL": (40.0, 38, 1520, "Backlog toward EUR120bn; capacity ramp delivers; margins near 19%"),
    "XBULL": (50.0, 40, 2000, "European rearmament supercycle fully funded through 2030+"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Vehicle Systems", "Tanks + armored vehicles (Panther, Lynx, Boxer) — the volume engine", 5.6, 5.0, 6.0),
    ("Weapon & Ammunition", "Artillery shells + propellants — highest margin, capacity-constrained", 4.4, 4.0, 4.8),
    ("Electronic Solutions", "Air defense, sensors, digitization — fastest-growing segment", 2.6, 2.3, 2.9),
    ("Power Systems", "Civil/automotive legacy — the non-defense rump", 1.4, 1.2, 1.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Revenue growth", "% YoY",
      10.0, 25.0, 45.0, 65.0, 69.0, True,
     "Order conversion slows; procurement bureaucracy bites"),

    ("Order backlog momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Backlog growth stalls as budgets get contested"),

    ("Operating margin", "%",
      10.0, 13.0, 16.0, 19.0, 17.1, True,
     "Ramp costs + fixed-price contracts squeeze margins"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Further order cancellations force more cuts"),

    ("Capacity ramp execution", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "New plants underdeliver; supply chain gates output"),

    ("European defense budgets", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Political shift or peace deal deflates the spending path"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("~44% below its 52-week high (EUR2,007) after the 2026 derate", 0.4, 0.2),
    ("Structural European rearmament — multi-year funded demand", 0.5, 0.2),
    ("~36x FY26E earnings — growth is heavily pre-paid even after the fall", -0.5, 0.25),
    ("Government-customer concentration — one cancellation already cut guidance", -0.4, 0.2),
    ("EUR80.5bn backlog (5.7x revenue) — visibility few industrials ever have", 0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 31.0   # FY2026E approx, EUR
EPP_MIN_PE       = 15.0    # defense floor — a funded backlog converts through any sentiment cycle

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 35.0, "+35% revenue (vs +69%; slower conversion)"),
    (1, 3.0, "Score 3/4 (vs 4; pace normalizes)"),
    (2, 15.5, "15.5% margin (vs 17.1; ramp costs)"),
    (3, 2.0, "Score 2/4 (held down)"),
    (4, 2.0, "Score 2/4 (vs 3; plants lag)"),
    (5, 3.0, "Score 3/4 (vs 4; budgets slower)"),
]
CONS_EPS_CAGR = 0.2
CONS_EXIT_PE  = 28.0
CONS_DIVIDEND = 11.5
...
print()
print("═" * W)
print(f"  RHM  ·  Rheinmetall AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Defense — Vehicles, Ammunition, Electronics")
...
print(f"  Q2 2026 (Aug 6): revenue EUR3.29bn +69%, op margin 17.1% (+3.7pp); record EUR80.5bn backlog; FY guidance trimmed EUR300M on a cancelled German order")
...
print("""
  KEY TRIGGER: a credible Ukraine ceasefire producing BOTH a sentiment derate
  AND real order deferrals — the multiple and the backlog conversion slowing
  together. JOINT event: NATO 3.5%-of-GDP commitments run to 2035 regardless;
  the EUR80.5bn backlog is contracted, not sentiment.""")
...
print("""
  Even at a 20% EPS CAGR and a 28x exit, the 2-yr return is only ~+13% with
  dividends — after a 44% drawdown the stock is finally paying for growth
  delivered, not growth promised.""")
...
print("""  45% realized vol and a EUR900-2,007 range: this is a policy-beta equity.
  The drawdown has moved the entry from euphoric to merely demanding.""")
