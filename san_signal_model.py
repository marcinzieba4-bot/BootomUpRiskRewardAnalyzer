#!/usr/bin/env python3
"""
Santander Signal Model  v1
──────────────────────
Banco Santander, S.A. (NYSE ADR: SAN)  ·  Global Retail & Commercial Banking
Trough year: 2023 (rate-shock deposit repricing fears)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 14.50    # USD, NYSE ADR, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR":  ( 9.50,  7,   8.50, "LatAm credit cycle turns; NIM compresses on rate cuts; provisions spike"),
    "BASE":  (13.50,  9,  16.00, "Underlying EPS growth continues at high-single/low-double digit"),
    "BULL":  (18.00, 11,  20.50, "Record profitability sustains; buybacks accelerate; LatAm outperforms"),
...
SIGNALS = [
    ("Underlying EPS growth YoY",      "% YoY",
      -5.0,   5.0,  12.0,  20.0,  17.0, True,
     "NII compression + higher provisions erase underlying earnings growth"),

    ("Net interest margin trend",      "/4 scale",
      1.0,   2.0,   3.0,   4.0,   3.0, True,
     "Central bank rate cuts compress NIM faster than deposit repricing offsets"),

    ("Cost of risk (credit quality)",  "/4 scale (inv)",
      1.0,   2.0,   3.0,   4.0,   3.0, True,
     "Brazil/Mexico consumer credit cycle turns; provisions rise sharply"),

    ("CET1 capital ratio",             "%",
      11.5,  12.3,  12.8,  13.5,  12.9, True,
     "Regulatory capital requirements tighten or losses erode capital buffer"),

    ("Return on tangible equity",      "%",
      9.0,  13.0,  16.0,  19.0,  16.5, True,
     "ROTE compresses back toward historical mid-single-digit-to-low-teens range"),

    ("Share buyback pace",             "/4 scale",
      1.0,   2.0,   3.0,   4.0,   3.0, True,
     "Capital return slows as regulators or macro conditions require capital conservation"),
]
WEIGHTS = [0.25, 0.20, 0.20, 0.10, 0.15, 0.10]

STRUCTURAL_FACTORS = [
    ("Record 2026 profitability + accelerating buybacks",        0.8, 0.30),
    ("Geographic diversification (Europe + LatAm + US)",         0.5, 0.20),
    ("LatAm (Brazil/Mexico) credit-cycle sensitivity",           -0.5, 0.25),
    ("Persistent valuation discount vs global bank peers",        0.4, 0.15),
    ("EUR-denominated earnings vs USD ADR — FX translation risk", -0.2, 0.10),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 1.30    # approx TTM EPS, USD (ADR basis)
EPP_MIN_PE       = 7.0     # min viable P/E at panic — bank-trough multiple
EPP_HISTORICAL   = 16.0    # historical EPP v1 (approx)

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
CONS_SIGNALS = [
    ("Underlying EPS",   6.0, "+6% YoY (vs +17%; growth normalizes off record 2026 base)"),
    ("Net interest",     2.0, "Score 2/4 (vs 3; modest NIM pressure from rate cuts)"),
    ("Cost of risk",     2.0, "Score 2/4 (vs 3; provisions normalize upward from cycle lows)"),
    ("CET1 capital",    12.5, "12.5% (vs 12.9%; buybacks continue but at a measured pace)"),
    ("Return on",       13.5, "13.5% ROTE (vs 16.5%; reverts toward long-run average)"),
    ("Share buyback",    2.0, "Score 2/4 (vs 3; pace slows but continues)"),
]
CONS_EPS_CAGR = 0.06     # 6%/yr conservative (vs recent underlying growth in high teens)
CONS_EXIT_PE  = 9.0      # 9x exit (still a discount to global bank peers)
CONS_DIVIDEND = 0.55     # approx USD0.55/yr cumulative annual dividend (ADR basis)
...
print(f"\n  Bear composite:  {bear_composite:.2f}  →  Bear scenario price: ~${expected_price(bear_probs):.2f}  (model)  /  ${SCENARIOS['BEAR'][2]} (defined)")
print(f"  Bear probability (proxy model):  {proxy_probs['BEAR']*100:.1f}%")
print(f"\n  KEY TRIGGER: Brazil/Mexico credit cycle turns (rising NPLs + provisions) AT THE")
print(f"  SAME TIME central bank rate cuts compress NIM → double earnings hit. JOINT event:")
print(f"  geographic diversification alone cushions a single-region shock, not both.")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          ${EPP_TODAY_EPS:.2f}  (approx TTM, USD ADR basis)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [bank-trough floor — diversified deposit franchise]")
print(f"  {'─'*60}")
print(f"  UPDATED EPP:                     ${epp_updated:.2f}/share")
print(f"  Historical EPP (v1, floor adj):  ${EPP_HISTORICAL:.2f}/share")
print(f"  Current ${CURRENT_PRICE:.2f} vs Updated EPP ${epp_updated:.2f}:  {epp_gap_pct:+.0f}%  {'✓ cushion' if epp_gap_pct >= 0 else '← in distressed zone'}")
print(f"  Bear ${SCENARIOS['BEAR'][2]} vs Updated EPP ${epp_updated:.2f}:  {bear_vs_epp_pct:+.0f}%  {'← BEAR requires earnings impairment' if bear_vs_epp_pct < 0 else '✓ bear is cyclical repricing'}")
...
print(f"  At {CONS_EXIT_PE:.0f}x P/E (conservative):  ${cons_price_2yr:.2f}/share")
if CONS_DIVIDEND > 0:
    print(f"  + Cumul. dividends (2yr):  +${cons_div_2yr:.2f}/share")
print(f"  {'─'*60}")
print(f"  Conservative 2yr price:     ${cons_price_2yr:.2f}  ({'▲' if cons_price_2yr > CURRENT_PRICE else '▼'}{abs(cons_price_2yr - CURRENT_PRICE):.2f} from ${CURRENT_PRICE:.2f})")
print(f"  Conservative total return:  {cons_total_ret:+.0f}% over 2yr  = {cons_annual_ret:+.0f}%/yr")
print(f"\n  Even at conservative 6% EPS growth and a 9x exit (still below global bank peers'")
print(f"  typical 10-12x), Santander's diversified franchise provides a floor most")
print(f"  single-region banks lack.")
...
print(f"  Realized vol (2yr):   {VOL_ANNUAL_PCT*100:.0f}% annualized  (elevated — EM bank + FX translation)")
print(f"  Beta vs broad market: {VOL_BETA:.2f}")
print(f"  1-sigma range (1yr):  ${vol_low_1yr:.2f}  –  ${vol_high_1yr:.2f}  (${CURRENT_PRICE:.2f} ± {VOL_ANNUAL_PCT*100:.0f}%)")
print(f"  2-sigma range (1yr):  ${CURRENT_PRICE - 2*sigma_1yr:.2f}  –  ${CURRENT_PRICE + 2*sigma_1yr:.2f}")
print(f"  {'─'*60}")
...
print(f"  Stock is up ~57% YTD (2026) on record profitability — near 52-week highs,")
print(f"  so much of the improving-fundamentals story is likely already priced in.")
...
print(f"\n  Proxy EV (2yr): ${proxy_ev:.2f}  /  Market EV: ${mkt_ev:.2f}  /  Current: ${CURRENT_PRICE:.2f}")
print(f"  Conservative EV (2yr, ④): ${cons_price_2yr:.2f} + ${cons_div_2yr:.2f} divs = ${cons_price_2yr + cons_div_2yr:.2f} total value")
