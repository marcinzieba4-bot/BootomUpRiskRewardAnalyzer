...
RMS Signal Model  v1
──────────────────────
Hermès International S.C.A. (EPA: RMS)  ·  Ultra-Luxury Goods (Leather, Fashion)
Trough year: 2020 (COVID store closures — briefest trough in luxury)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 1643.5   # EUR, Euronext Paris, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (33.0, 35, 1150, "Aspirational demand breaks; first sub-40% margin decade; multiple derates to 35x"),
    "BASE": (44.0, 39, 1700, "6-8% cc growth; 41% margin held; China stabilizes at low level"),
    "BULL": (50.0, 43, 2150, "China rebound + US strength; production capacity +7-8%/yr fully absorbed"),
    "XBULL": (57.0, 46, 2600, "Full luxury recovery; Hermès reasserts scarcity premium; re-rating toward 45x+"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Leather Goods & Saddlery", "Core (Birkin/Kelly): H1 EUR3.76bn +9.8% cc; production-constrained by design", 7.7, 7.2, 8.2),
    ("Ready-to-wear & Accessories", "Second pillar; tracks core with higher fashion cyclicality", 4.7, 4.3, 5.1),
    ("Silk, Textiles & Other", "Heritage categories; most gifting-dependent, weakest in downturns", 1.6, 1.4, 1.8),
    ("Perfume, Beauty & Watches", "Entry price points — the aspirational-customer barometer", 1.9, 1.7, 2.1),
    ("Jewelry & Home", "Fastest-growing small base; high-jewelry buildout continues", 1.0, 0.9, 1.2),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Group cc revenue growth", "% YoY",
      0.0, 5.0, 9.0, 13.0, 6.1, True,
     "First negative print of the modern era as aspirational demand breaks"),

    ("Leather Goods cc growth", "% YoY",
      2.0, 6.0, 10.0, 14.0, 9.8, True,
     "Production-constrained core decelerates below 2%"),

    ("Recurring operating margin", "%",
      36.0, 39.0, 41.0, 43.0, 41.0, True,
     "Deleverage + FX compress the 40%+ margin fortress"),

    ("Asia-Pacific ex-Japan growth", "% YoY",
      -5.0, 1.0, 5.0, 10.0, 2.5, True,
     "China luxury malaise deepens; Korea rolls over"),

    ("Americas demand momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "US wealth-effect demand cracks"),

    ("Annual pricing power", "%/yr",
      2.0, 4.0, 7.0, 10.0, 6.0, True,
     "Price increases meet real resistance for the first time"),

]
WEIGHTS = [0.25, 0.20, 0.20, 0.15, 0.10, 0.10]

STRUCTURAL_FACTORS = [
    ("Absolute pricing power + waitlist economics (Birkin/Kelly)", 1.2, 0.3),
    ("Craftsman capacity constraint caps supply growth ~7%/yr", -0.3, 0.15),
    ("~39x trailing EPS — still premium-priced after a -32% 12-mo derate", -0.8, 0.25),
    ("Family-controlled, net cash, zero leverage risk", 0.5, 0.15),
    ("China stabilizing but 'no fundamental rebound' (CEO Dumas)", -0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 41.7   # TTM reported, EUR
EPP_MIN_PE       = 25.0    # ultra-luxury moat floor — scarcity model survives any cycle

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 4.5, "+4.5% cc (vs +6.1%; slow luxury)"),
    (1, 7.0, "+7% leather (vs +9.8%)"),
    (2, 40.0, "40% margin (vs 41%; mild FX drag)"),
    (3, 0.0, "flat APAC (vs +2.5%; China stuck)"),
    (4, 2.0, "Score 2/4 (vs 3; US normalizes)"),
    (5, 5.0, "+5% pricing (vs +6%)"),
]
CONS_EPS_CAGR = 0.08
CONS_EXIT_PE  = 32.0
CONS_DIVIDEND = 16.0
...
print()
print("═" * W)
print(f"  RMS  ·  Hermès International S.C.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Ultra-Luxury Goods (Leather, Fashion)")
...
print(f"  H1 2026 actual: revenue EUR8.16bn +6.1% cc (+1.6% reported), recurring operating margin 41.0%")
...
print("""
  KEY TRIGGER: the aspirational customer breaking (entry categories first — watch
  Perfume/Watches) WHILE core waitlist demand thins in China and the US together.
  JOINT event: scarcity economics protect the core against any single-region shock.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [ultra-luxury moat floor — scarcity model survives any cycle]")
print(f"  {'─'*60}")
...
print("""
  Even at 8% EPS CAGR and a 32x exit (vs 39x today), the 2-yr return is roughly
  flat — Hermès rarely gets cheap; it gets less expensive, then re-rates.""")
...
print("""  Down ~32% over 12 months yet still at 39x — the derate has removed froth, not
  premium. 52-week high shown is approximate given the early-2025 peak distortion.""")
