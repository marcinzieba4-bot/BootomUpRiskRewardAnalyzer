"""
MAR  ·  Marriott International, Inc.  ·  NASDAQ: MAR
...
SECTOR        = "Lodging · Asset-Light Franchise/Management · NASDAQ: MAR"
CURRENT_PRICE = 290.00      # USD; as of 2026-06-11
VOL_52W_LOW   = 215.00
VOL_52W_HIGH  = 305.00
SHARES_OUT_M  = 290.0       # millions; ongoing buyback

# Dividend
ANNUAL_DIV    = 2.40        # $/share
...
FEE_MARGIN           = 0.60  # incremental margin on fee revenue is very high
TAX_RATE             = 0.24

# ── EPP (Earnings Power Price) ────────────────────────────────────────────────
EPP_EPS   = 9.50    # FY2026E adj EPS
PE_TROUGH = 15      # min P/E at crisis trough (2020 lows on normalized EPS)
...
# ── SCENARIO TABLE ────────────────────────────────────────────────────────────
SCENARIOS = {
    "BEAR":  ( 7.50, 18,  135, "Global travel demand shock (recession/geopolitical); RevPAR negative; EPS $7.50 -> 18x distress P/E"),
    "BASE":  ( 9.50, 30,  285, "RevPAR +2-4%; net rooms growth ~5% drives high-margin fee growth; EPS $9.50 -> 30x P/E"),
    "BULL":  (11.00, 33,  363, "Travel demand re-accelerates; loyalty program (Bonvoy) drives direct-booking mix higher; EPS $11.00 -> 33x premium P/E"),
    "XBULL": (13.00, 36,  468, "Multi-year global travel supercycle; unit growth accelerates in Asia/Middle East; EPS $13.00 -> 36x peak P/E"),
}
...
SIGNALS = [
    ...
    {
        "name":       "Loyalty program engagement (Bonvoy direct-booking mix)",
        "weight":     0.15,
        "thresholds": ("<55%",   "≥60%",  "≥65%",   "≥70%"),
        "now":        "~64%",
        "score":      3,
        "comment":    "~64%; ~235M Bonvoy members lower distribution costs and improve owner economics",
    },
    {
...
SCA_FACTORS = [
    ...
    ("+", "Largest global hotel footprint + Bonvoy loyalty network — distribution and brand scale advantages compound over time",      +0.5, 0.20),
...
CONS_EPS_2YR = 11.00    # conservative FY2028E: unit growth + buyback compounding continues
...
print()
print("  REVPAR GROWTH vs UNIT GROWTH ANALYSIS  (the core MAR earnings drivers)")
hr()
shares_b = SHARES_OUT_M / 1000
total_fee_rev_b = 6.0
print(f"  Estimated FY2026E fee revenue base: ~${total_fee_rev_b:.1f}B")
print()
print(f"  REVPAR GROWTH UPSIDE  (incremental EPS from RevPAR growth at {FEE_MARGIN*100:.0f}% incremental margin):")
print(f"  {'RevPAR YoY growth':<20}  {'Rev/yr ($B)':>12}  {'EPS/yr':>10}  {'2yr EPS':>10}")
hr()
for g in [0.00, 0.02, 0.03, 0.05]:
    rev_inc = total_fee_rev_b * g
    inc_eps = rev_inc * FEE_MARGIN * (1 - TAX_RATE) / shares_b
    eps_2yr = inc_eps * 2
...
bear_triggers = [
    ...
    ("Bonvoy direct-booking mix",                 "~64%",   "<55%",  "-9pp",   "OTA share regains ground, raising distribution costs"),
...
hr()
print(f"  Bear ${bear_price} requires:  ~{bear_sigmas:.1f}σ move  (2020 MAR fell ~70% from peak intra-COVID; tail risk, not base case)")
print(f"  -> Global travel demand trajectory is the PRIMARY catalyst; resilient travel = upside;")
...
print(f"  -> BUY $230-$250  |  TRIM $310+  |  AVOID above $340")
...
print(f"  (3) Bonvoy direct-booking mix -> continued gains lower distribution costs")
print(f"  (4) Group/business travel recovery -> approach/exceed 2019 levels confirms BULL case")
print(f"  BUY $230-{round(CURRENT_PRICE*0.90,0):.0f}  |  ACCUMULATE to ${round(CURRENT_PRICE*1.05,0):.0f}  |  TRIM above $310  |  AVOID above $340")
print("═" * (W + 4))
print()
