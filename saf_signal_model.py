...
SAF Signal Model  v1
──────────────────────
Safran S.A. (EPA: SAF)  ·  Aerospace Propulsion & Equipment
Trough year: 2020 (COVID air-traffic collapse)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 342.7   # EUR, Euronext Paris, 2026-08-20
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (7.5, 32, 240, "Air-traffic shock; LEAP aftermarket ramp stalls; supply chain caps output"),
    "BASE": (11.0, 31, 340, "Guidance delivered: ROI EUR6.4-6.5bn, FCF EUR4.7-4.9bn; aftermarket compounds"),
    "BULL": (13.5, 32, 430, "LEAP shop-visit wave accelerates; margins beat; defense orders build"),
    "XBULL": (16.0, 32, 510, "Aftermarket supercycle + defense rearmament + pricing power compound"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Propulsion", "LEAP + CFM56 installed base — the aftermarket annuity; H1 LEAP deliveries +41%", 18.0, 16.0, 19.5),
    ("Equipment & Defense", "Landing systems, avionics, defense electronics — rearmament tailwind", 11.5, 10.5, 12.5),
    ("Aircraft Interiors", "Seats/cabins — recovering with widebody build rates", 3.5, 3.2, 3.9),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Organic revenue growth", "% YoY",
      5.0, 10.0, 16.0, 22.0, 20.2, True,
     "Air traffic stalls; spares/aftermarket demand rolls over"),

    ("LEAP delivery growth", "% YoY",
      0.0, 10.0, 25.0, 40.0, 41.0, True,
     "Supply chain caps engine output again"),

    ("Civil aftermarket momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Shop-visit wave defers; spares pricing softens"),

    ("Recurring operating margin", "%",
      14.0, 16.0, 18.0, 20.0, 18.4, True,
     "Mix shifts to lower-margin OE deliveries"),

    ("FY ROI guidance (mid)", "EURbn",
      5.8, 6.1, 6.4, 6.7, 6.45, True,
     "Guidance cut on supply-chain or traffic shock"),

    ("FY FCF guidance (mid)", "EURbn",
      3.8, 4.2, 4.6, 5.0, 4.8, True,
     "Working capital absorbs the cash conversion"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("CFM56/LEAP duopoly installed base — decades-long aftermarket annuity", 1.0, 0.3),
    ("Guidance RAISED +EUR300M at H1 — momentum validated", 0.4, 0.15),
    ("~31x FY2026E EPS near 52-week high — execution premium priced", -0.7, 0.25),
    ("Engine supply-chain fragility caps upside delivery", -0.4, 0.15),
    ("European defense rearmament second engine", 0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 11.0   # FY2026E recurring, approx EUR
EPP_MIN_PE       = 16.0    # aftermarket-annuity floor — installed base survives any cycle

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 12.0, "+12% organic (vs +20.2%; normalization)"),
    (1, 15.0, "+15% LEAP (vs +41%; supply caps)"),
    (2, 2.0, "Score 2/4 (vs 3; visits defer)"),
    (3, 17.0, "17% margin (vs 18.4%; OE mix)"),
    (4, 6.2, "EUR6.2bn ROI (vs 6.45; low end)"),
    (5, 4.4, "EUR4.4bn FCF (vs 4.8)"),
]
CONS_EPS_CAGR = 0.1
CONS_EXIT_PE  = 24.0
CONS_DIVIDEND = 3.35
...
print()
print("═" * W)
print(f"  SAF  ·  Safran S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Aerospace Propulsion & Equipment")
...
print(f"  H1 2026 actual: revenue EUR17.57bn +19% (+20.2% organic), recurring op income EUR3.24bn +29% (18.4% margin)")
...
print("""
  KEY TRIGGER: a global air-traffic shock (the only force that stops the
  aftermarket annuity) COINCIDING with supply-chain failure on LEAP output —
  demand and delivery failing together. JOINT event: the installed base cushions either.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (FY2026E recurring, approx EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [aftermarket-annuity floor — installed base survives any cycle]")
print(f"  {'─'*60}")
...
print("""
  At 10% EPS CAGR and a 24x exit (vs ~31x today), the 2-yr return is slightly
  negative — the annuity quality is real, and so is the multiple paid for it.""")
...
print("""  Sits ~7% below its 52-week high after a raised-guidance H1; the aftermarket
  annuity makes this the most defensive of the aerospace names on fundamentals.""")
