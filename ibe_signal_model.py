...
IBE Signal Model  v1
──────────────────────
Iberdrola, S.A. (BME: IBE)  ·  Regulated Networks & Renewables Utility
Trough year: 2022 (rate shock + Spanish windfall-tax fears)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 20.15   # EUR, BME Madrid, 2026-08 (mid-month)
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (0.65, 24, 15.5, "Rate spike + regulatory reset in UK/US; renewables returns compress"),
    "BASE": (0.9, 22, 20, "Networks-led >8% adjusted profit growth delivered; capex plan executes"),
    "BULL": (1.05, 23, 24, "Rate cuts + tariff resets compound; grid capex supercycle re-rates the stock"),
    "XBULL": (1.2, 23, 28, "Full utility re-rating as electrification demand outruns supply"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E adjusted EBITDA, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Networks (regulated)", "Transmission/distribution UK-US-Spain-Brazil; H1 EBITDA +13% on tariff resets", 9.0, 8.6, 9.5),
    ("Renewables", "Offshore/onshore wind + solar; returns tied to PPA prices and rates", 6.0, 5.2, 6.8),
    ("Customer & Other", "Retail supply + flexible generation", 2.5, 2.3, 2.7),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Adjusted net profit growth", "% YoY",
      0.0, 5.0, 8.0, 12.0, 8.0, True,
     "Regulatory resets + rate costs stall profit growth"),

    ("Networks EBITDA growth", "% YoY",
      2.0, 6.0, 10.0, 14.0, 13.0, True,
     "Tariff reviews turn hostile in UK/US/Brazil"),

    ("Capex deployment", "EURbn/yr",
      9.0, 11.0, 13.0, 15.0, 14.0, True,
     "Funding costs force capex plan cuts"),

    ("Regulated share of EBITDA", "%",
      40.0, 48.0, 55.0, 62.0, 55.0, True,
     "Merchant/renewables mix rises, raising risk profile"),
    ...
    ("Renewables output & pricing", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "Wind resource + captured prices disappoint"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Regulated-networks growth visibility (tariff-driven, inflation-linked)", 0.7, 0.3),
    ("~25x trailing EPS near 52-week high — rich for a utility", -0.6, 0.25),
    ("Rate sensitivity of both valuation and funding costs", -0.4, 0.2),
    ("Proven renewables developer at global scale", 0.3, 0.15),
    ("H1 profit flattered by one-off Mexican asset-sale gain", -0.2, 0.1),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 0.82   # TTM reported, EUR
EPP_MIN_PE       = 14.0    # regulated-utility floor — networks RAB underpins

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% adj profit (vs +8%; guidance floor)"),
    (1, 7.0, "+7% networks (vs +13%)"),
    (2, 12.0, "EUR12bn capex (vs 14; funding costs)"),
    (3, 52.0, "52% regulated (vs 55%)"),
    (4, 2.0, "Score 2/4 (vs 2.5; rates sticky)"),
...
print()
print("═" * W)
print(f"  IBE  ·  Iberdrola, S.A.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Regulated Networks & Renewables Utility")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E adjusted EBITDA, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  H1 2026: net profit +22% to EUR4.34bn (incl. EUR950M Mexico sale gain; adjusted +8%); capex +25% to EUR7bn")
...
print("""
  KEY TRIGGER: a rates re-spike hitting the utility discount rate WHILE UK/US
  regulators turn hostile on tariff resets — valuation and earnings hit at once.
  JOINT event: the regulated RAB model absorbs either shock alone.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [regulated-utility floor — networks RAB underpins]")
print(f"  {'─'*60}")
...
print("""
  At 6% EPS CAGR and an 18x exit (vs ~25x today), the 2-yr return is negative —
  the growth story is real, but a utility at these multiples has no valuation slack.""")
...
print("""  Lowest-volatility name in the additions (17%), but note: low vol cuts both
  ways — the 15%/yr hurdle is structurally hard for a utility to clear.""")
