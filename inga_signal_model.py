#!/usr/bin/env python3
"""
INGA Signal Model  v1
──────────────────────
ING Groep N.V. (AMS: INGA)  ·  Pan-European Retail & Wholesale Banking
Trough year: 2020 (COVID provisions + dividend ban)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 30.67   # EUR, Euronext Amsterdam, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (2.0, 10, 20, "ECB cuts crush deposit margins; fee growth stalls; provisions turn"),
    "BASE": (2.5, 12, 30, "Raised guidance lands: income >EUR24.5bn, ROTE >15%"),
    "BULL": (2.8, 13, 36, "Fee engine + volume growth offset NII; ROTE >16% sustained"),
    "XBULL": (3.1, 14, 43, "Eurozone deposit-franchise re-rating; digital-scale premium"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E total income, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Retail Netherlands & Belgium", "Home-market deposit franchise — the margin engine now under ECB pressure", 9.6, 9.0, 10.1),
    ("Retail Rest of World", "Germany/Spain/Poland/Australia digital-first franchises — the growth layer", 7.3, 6.8, 7.8),
    ("Wholesale Banking", "Lending + financial markets; disciplined RWA diet", 8.1, 7.4, 8.7),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Net profit growth", "% YoY",
      -5.0, 5.0, 12.0, 20.0, 16.0, True,
     "Deposit-margin compression + provisions reverse the run"),

    ("Total income growth", "% YoY",
      0.0, 3.0, 6.0, 10.0, 10.2, True,
     "NII fade outruns fee and volume growth"),

    ("Fee income momentum", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Investment-product and payments fees stall"),

    ("CET1 ratio", "%",
      12.5, 13.0, 13.4, 13.8, 13.3, True,
     "Buybacks + RWA growth burn the buffer faster"),

    ("Deposit margin resilience", "/4",
      1.0, 2.0, 3.0, 4.0, 2.5, True,
     "ECB cuts compress replicating-portfolio yields"),

    ("Capital distributions", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Distribution pace throttled to defend CET1"),

]
WEIGHTS = [0.20, 0.20, 0.15, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Digital-first cost model — best efficiency in eurozone retail", 0.4, 0.2),
    ("At the 52-week high after a +52% run from the low", -0.7, 0.25),
    ("Direct ECB rate-cut sensitivity — deposit margins are the P&L", -0.5, 0.2),
    ("Structural distribution machine (dividends + rolling buybacks)", 0.4, 0.15),
    ("Guidance RAISED at Q2 — momentum validated", 0.3, 0.2),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 2.31   # TTM reported, EUR
EPP_MIN_PE       = 6.0    # Benelux-bank trough floor — deposit franchise survives rate cycles

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 5.0, "+5% profit (vs +16%; NII fades)"),
    (1, 4.0, "+4% income (vs +10.2%)"),
    (2, 2.0, "Score 2/4 (vs 3; fees plateau)"),
    (3, 13.0, "13.0% CET1 (vs 13.3)"),
    (4, 2.0, "Score 2/4 (vs 2.5; cuts land)"),
    (5, 2.0, "Score 2/4 (vs 3; pace slows)"),
]
CONS_EPS_CAGR = 0.03
CONS_EXIT_PE  = 9.0
CONS_DIVIDEND = 1.16
...
print()
print("═" * W)
print(f"  INGA  ·  ING Groep N.V.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Pan-European Retail & Wholesale Banking")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E total income, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026: net profit EUR1.95bn +16%; PBT +23%; income +10.2%; FY guidance RAISED — income >EUR24.5bn, ROTE >15%")
...
print("""
  KEY TRIGGER: the ECB cutting cycle compressing deposit margins ACROSS every
  ING market at once — the one risk a geographically diversified deposit bank
  cannot diversify away. JOINT event only with fees stalling simultaneously.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [Benelux-bank trough floor — deposit franchise survives rate cycles]")
print(f"  {'─'*60}")
...
print("""
  At 3% EPS CAGR and a 9x exit, the 2-yr price return is negative from the
  high — the raised guidance is real, and the price has already collected it.""")
...
print("""  Within ~1.5% of its 52-week high (EUR31.11) after a +52% run; a pure
  rate-cycle bank at the top of its range late in the hiking dividend.""")
