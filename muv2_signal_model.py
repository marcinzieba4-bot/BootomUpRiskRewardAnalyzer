#!/usr/bin/env python3
"""
MUV2 Signal Model  v1
──────────────────────
Münchener Rück AG (Munich Re) (XETR: MUV2)  ·  Reinsurance — P&C, Life/Health, ERGO Primary
Trough year: 2020 (COVID claims + capital-market shock)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 513.9   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (42.0, 10, 420, "Soft cycle + mega-cat year; pricing and losses turn together"),
    "BASE": (54.0, 10, 540, "EUR6.3bn net result lands; softening priced into renewals rationally"),
    "BULL": (60.0, 10, 620, "Pricing stabilizes; low cat years compound the capital machine"),
    "XBULL": (66.0, 11, 730, "Hard market returns (post-event); Munich Re re-rates as the quality name"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E insurance revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("P&C Reinsurance", "The profit engine — pricing now SOFTENING after the 2023-25 hard market", 26.0, 24.0, 27.5),
    ("Life & Health Reinsurance", "Mortality/morbidity book — steady compounder, less cyclical", 12.0, 11.5, 12.7),
    ("ERGO (primary)", "German + international primary insurance — reliable, unspectacular", 22.0, 21.0, 23.0),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("FY net-result progress", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "H2 cat season erases the H1 cushion"),

    ("P&C pricing cycle", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "July renewals confirm accelerating rate softening"),

    ("Major-loss experience", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Hurricane/quake cluster blows the major-loss budget"),

    ("Return on equity", "%",
      10.0, 14.0, 18.0, 22.0, 23.0, True,
     "ROE mean-reverts as pricing and luck normalize"),

    ("Revenue trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Guidance cut deepens as underwriting discipline holds"),

    ("ERGO performance", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "German motor/health margins compress"),

]
WEIGHTS = [0.20, 0.15, 0.15, 0.15, 0.20, 0.15]

STRUCTURAL_FACTORS = [
    ("Softening reinsurance cycle — revenue guidance already cut EUR2bn", -0.6, 0.25),
    ("~9.6x earnings + 4.7% dividend — priced like a decline, run like a machine", 0.5, 0.2),
    ("Underwriting discipline: walks from underpriced business (the cut IS discipline)", 0.4, 0.2),
    ("H1 flattered by very low major losses — luck, not run-rate", -0.4, 0.2),
    ("~11% below its 52-week high; beta 0.75 defensive profile", 0.2, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 53.72   # TTM reported, EUR
EPP_MIN_PE       = 7.0    # reinsurance-trough floor — fortress capital re-prices post-event

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 3.0, "Score 3/4 (vs 4; normal cat H2)"),
    (1, 2.0, "Score 2/4 (softening persists)"),
    (2, 2.0, "Score 2/4 (vs 4; budget consumed)"),
    (3, 16.0, "16% ROE (vs 23; normalization)"),
...
print()
print("═" * W)
print(f"  MUV2  ·  Münchener Rück AG (Munich Re)  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Reinsurance — P&C, Life/Health, ERGO Primary")
print(f"  Verdict: {_verdict}  ·  Adj gap {adj_gap:+.2f}")
print("═" * W)

print(f"\n  SEGMENT REVENUE BRIDGE  (FY2026E insurance revenue, EUR bn  →  BEAR / BULL scenarios)")
print("  " + "─" * (W-2))
...
print(f"  Q2 2026: net profit ~EUR2.2bn (cons. EUR1.79bn); H1 EUR3.9bn = 62% of the EUR6.3bn FY target; reinsurance revenue guidance CUT EUR2bn to EUR38bn")
...
print("""
  KEY TRIGGER: the soft-pricing phase COLLIDING with a record catastrophe year
  — rates falling into the loss event instead of after it, compressing both
  sides of the underwriting equation. JOINT event: L&H Re + ERGO + investment
  income carry the result through a single bad season.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [reinsurance-trough floor — fortress capital re-prices post-event]")
print(f"  {'─'*60}")
...
print("""
  At 4% EPS CAGR and a 9x exit, the 2-yr total return is modestly positive on
  the EUR24 dividend alone — the equity asks little; the cycle is the risk.""")
...
print("""  Beta 0.75 and an 18% vol on a EUR64bn reinsurer: the defensive shape is
  real, but soft-cycle years historically compress the multiple before losses show.""")
