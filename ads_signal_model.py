...
ADS Signal Model  v1
──────────────────────
adidas AG (XETR: ADS)  ·  Sportswear — Footwear & Apparel
Trough year: 2022 (Yeezy crisis)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 156.25   # EUR, Xetra, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (5.0, 21, 105, "Post-World-Cup hangover; US tariffs bite; Nike counterattack compresses margins"),
    "BASE": (7.5, 22, 165, "9-10% cc growth lands; op profit ~EUR2.3bn; brand heat sustains"),
    "BULL": (9.0, 24, 216, "Margin catch-up toward 10% — the structural gap vs peers closes"),
    "XBULL": (11.0, 25, 275, "Full brand supercycle + US recovery + operating leverage compound"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Footwear", "Samba/Gazelle/Spezial lifestyle wave + running (Adizero) — the heat engine", 15.5, 14.5, 16.5),
    ("Apparel", "Jerseys + athleisure; World Cup 2026 tailwind year", 8.5, 8.0, 9.2),
    ("Accessories & other", "Balls, bags, licensing", 2.5, 2.3, 2.8),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Revenue growth (cc)", "% YoY",
      0.0, 5.0, 10.0, 15.0, 14.0, True,
     "Lifestyle franchises cool; World Cup demand laps out"),

    ("Guidance trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 4.0, True,
     "Guidance cut as US/tariff pressure builds"),

    ("Earnings delivery vs consensus", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "More EPS misses — costs keep eating the topline beats"),

    ("Gross margin", "%",
      47.0, 49.5, 51.5, 53.5, 52.0, True,
     "Tariffs + discounting reverse the margin build"),

    ("Brand heat & franchise breadth", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Terrace trend fades before new franchises scale"),

    ("US / tariff exposure", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "Tariff costs + soft US consumer squeeze the growth market"),

]
WEIGHTS = [0.20, 0.15, 0.20, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("Post-miss reset: -16% single-day drop has cleared the froth", 0.3, 0.2),
    ("Brand momentum: record sales, +14% cc, upgraded guidance", 0.4, 0.2),
    ("EPS conversion problem — sales records not reaching the bottom line", -0.4, 0.25),
    ("Nike's recovery cycle is the direct competitive risk into 2027", -0.3, 0.2),
    ("Mid-range entry (EUR130-196 band) — neither euphoric nor distressed", 0.1, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 6.0   # TTM approx, EUR
EPP_MIN_PE       = 14.0    # brand floor — global #2 sportswear does not trade at retailer multiples

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 7.0, "+7% cc (vs +14; laps World Cup)"),
    (1, 3.0, "Score 3/4 (vs 4; held)"),
    (2, 2.0, "Score 2/4 (conversion stays poor)"),
    (3, 50.5, "50.5% GM (vs 52; tariffs)"),
    (4, 2.0, "Score 2/4 (vs 3; trend cools)"),
    (5, 2.0, "Score 2/4 (pressure persists)"),
]
CONS_EPS_CAGR = 0.1
CONS_EXIT_PE  = 20.0
CONS_DIVIDEND = 2.0
...
print()
print("═" * W)
print(f"  ADS  ·  adidas AG  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Sportswear — Footwear & Apparel")
...
print(f"  Q2 2026 (Jul 30): record Q2 — revenue +14% cc (beat), gross margin >52%, but EPS missed and the stock fell 16%; FY revenue guidance UPGRADED to 9-10% cc, op profit ~EUR2.3bn")
...
print("""
  KEY TRIGGER: the terrace/lifestyle wave fading in the SAME year US tariffs
  and a Nike counterattack land — demand heat and margin structure failing
  together. JOINT event: franchise breadth (running, football, originals)
  cushions any single trend's decay.""")
...
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [brand floor — global #2 sportswear does not trade at retailer multiples]")
print(f"  {'─'*60}")
...
print("""
  At 10% EPS CAGR and a 20x exit, the 2-yr return is slightly negative — the
  brand story is strong; the earnings conversion has to start matching it.""")
...
print("""  A 16% single-day drop on an EPS miss in a record quarter: the market is
  policing conversion, not growth. The reset entry is the opportunity IF
  margins follow sales.""")
