...
WKL Signal Model  v1
──────────────────────
Wolters Kluwer N.V. (AMS: WKL)  ·  Professional Information & Software — Health, Tax, Legal, Compliance
Trough year: 2026 (AI-disruption derate — the current drawdown IS the trough multiple)
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
CURRENT_PRICE    = 69.5   # EUR, Euronext Amsterdam, 2026-08-26
REQUIRED_RETURN  = 0.15
HORIZON_YEARS    = 2

SCENARIOS = {
    "BEAR": (4.8, 10, 48, "AI genuinely disintermediates professional content; churn rises; pricing power breaks"),
    "BASE": (6.2, 12, 74, "Guidance delivered: solid organic growth, higher margins, HSD EPS growth"),
    "BULL": (7.0, 14, 98, "AI products (embedded in workflows) prove accretive; multiple partially rebuilds"),
    "XBULL": (8.0, 16, 128, "AI fear fully refuted — recurring software re-rating toward historic multiples"),
}

# ── SEGMENT REVENUE BRIDGE (FY2026E revenue, EUR bn) ────────────────────────────────
SEGMENTS = [
    ("Tax & Accounting", "CCH tax/audit software — workflow-embedded, highest stickiness", 1.75, 1.68, 1.85),
    ("Health", "UpToDate clinical decisions + Ovid — the AI-debate epicenter", 1.55, 1.45, 1.62),
    ("Financial & Corp Compliance", "Lien search, incorporations, finance/risk software", 1.5, 1.42, 1.58),
    ("Legal & Regulatory", "Legal research + EHS/ESG software; print decline drag", 0.95, 0.88, 1.0),
    ("Corporate Performance & ESG", "CCH Tagetik + Enablon — fastest-growing division", 0.45, 0.42, 0.5),
]

# ── SIGNALS ───────────────────────────────────────────────────────────────────
SIGNALS = [
    ("Organic revenue growth", "% YoY",
      1.0, 3.0, 5.0, 7.0, 5.0, True,
     "AI substitution starts showing in renewal rates"),

    ("Recurring cloud software growth", "% YoY",
      4.0, 8.0, 12.0, 16.0, 14.0, True,
     "Cloud migration stalls; competitors take workflow share"),

    ("Adjusted EPS growth (cc)", "% YoY",
      0.0, 6.0, 10.0, 14.0, 14.0, True,
     "Margin expansion reverses on AI investment needs"),

    ("Margin trajectory", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "Product-development step-up (12-13% of revenue) eats margins"),

    ("AI disruption vs AI product proof", "/4",
      1.0, 2.0, 3.0, 4.0, 2.0, True,
     "GenAI answers displace UpToDate/CCH lookups at the margin"),

    ("Guidance delivery", "/4",
      1.0, 2.0, 3.0, 4.0, 3.0, True,
     "More divisional trims follow the small one in H1"),

]
WEIGHTS = [0.20, 0.15, 0.20, 0.15, 0.15, 0.15]

STRUCTURAL_FACTORS = [
    ("-41% from its 52-week high; ~11.8x earnings for 85% recurring revenue", 0.6, 0.25),
    ("AI-disintermediation risk is genuine and unresolved — this is not noise", -0.5, 0.25),
    ("Workflow embedding (not content alone) is the defensible layer", 0.3, 0.2),
    ("Buyback + 3.6% growing dividend while the debate rages", 0.3, 0.15),
    ("Fundamentals still printing: +14% cc EPS, +14% cloud growth", 0.4, 0.15),
]

# ── UPDATED EPP ───────────────────────────────────────────────────────────────
EPP_TODAY_EPS    = 5.88   # TTM adjusted, EUR
EPP_MIN_PE       = 10.0    # recurring-revenue floor — priced as if AI wins; renewals say otherwise

# ── CONSERVATIVE GROWTH (2-yr, base-minus) ───────────────────────────────────
# (signal_index, conservative_value, rationale)
CONS_SIGNALS = [
    (0, 3.5, "+3.5% organic (vs +5; AI drag starts)"),
    (1, 10.0, "+10% cloud (vs +14)"),
    (2, 8.0, "+8% EPS (vs +14; investment step-up)"),
    (3, 2.0, "Score 2/4 (vs 3; margins flat)"),
    (4, 2.0, "Score 2/4 (debate unresolved)"),
    (5, 2.0, "Score 2/4 (more trims)"),
]
CONS_EPS_CAGR = 0.07
CONS_EXIT_PE  = 12.0
CONS_DIVIDEND = 2.52
...
print()
print("═" * W)
print(f"  WKL  ·  Wolters Kluwer N.V.  ·  {CUR}{CURRENT_PRICE:,.2f}  ·  Professional Information & Software — Health, Tax, Legal, Compliance")
...
print(f"  H1 2026 (Aug 5): revenue EUR3.03bn, organic +5% (+6% ex-print); recurring (85% of total) +7%; cloud software +14%; adj EPS EUR2.83 +14% cc; FY guidance REITERATED")
...
print("""
  KEY TRIGGER: renewal-rate deterioration in Health (UpToDate) — the first
  hard evidence of AI substitution — arriving WHILE the product-development
  step-up compresses margins: revenue and margin failing together. JOINT
  event: Tax/Compliance workflows are regulatory-moated against pure GenAI.""")

print(f"\n  ③ UPDATED EPP  (floor anchored on TODAY's fundamentals × trough multiple)")
print("  " + "─" * (W-2))
print(f"  Today's normalized EPS:          {CUR}{EPP_TODAY_EPS:.2f}  (TTM adjusted, EUR)")
print(f"  Min viable P/E at panic:          {EPP_MIN_PE:.0f}x  [recurring-revenue floor — priced as if AI wins; renewals say otherwise]")
print(f"  {'─'*60}")
...
print("""
  At 7% EPS CAGR and a 12x exit, the 2-yr total return is ~+24% — the market
  has priced a disruption that the reported numbers have not yet shown.""")
...
print("""  From EUR117 to EUR69 in a year on a thesis, not a P&L: every reported
  quarter (+14% EPS cc) has beaten the fear. The gap between narrative and
  numbers is the position.""")
